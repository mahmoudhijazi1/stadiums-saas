# Stadium SaaS â progress & learning log

This is the **story of what we actually built**, in order, in plain language.

It is for you: to stay current, to explain a file to yourself a month from now, and to **defend a decision** in an interview or a review (âwe did X because DR-002 said Y, not because the tutorial didâ).

It is **not** the source of truth for product or architecture. Those stay in:

| Document | Job | Example |
|---|---|---|
| [BRD](./requirements/brd.md) | What the business needs | âTwo confirmed bookings never overlapâ |
...
| **This file** | What we did, when, which files, how they connect | âproxy sets a header because Next middleware cannot share memory with the pageâ |
...
3. For âwhy not the other design?â, jump to the linked DR â this log only *points* there.
...
**What they cannot do yet:** refunds, per-player split, pitch blocks, subscriptions/suspend, other WhatsApp templates (reject/cancel/reminder), public location/contact/QR, games-played / pitch-busy (BR-57). Ops: no written backup cadence; prod env checklist still thin â see [NOW.md](./NOW.md).
...
**Next:** MVP P0 remaining â backup cadence, then deploy/prod env truth ([NOW.md](./NOW.md)).
...
That paragraph lives in the BRD. Code so far only proves: *the right stadiumâs data, and a day of slots from a rule, not a slots table.*

---

## Stack â what we chose and why it shows up in files

| Piece | Why it is here | Where you see it |
|---|---|---|
| **Next.js 16** (App Router) | UI + server in one repo. **This version is not âthe Next.js you memorized.â** `AGENTS.md` + `.cursor/rules`: read `node_modules/next/dist/docs/` before APIs. | `src/app/`, `src/proxy.ts` (not `middleware.ts`) |
...
## Chapter 0 â create-next-app and Prisma (before SPEC-01)

**Commits:** `ceb9d55` Create Next App â `413d3bc` move to `src/` and Prisma.

**What:** A stock Next app, then the project was moved so source lives under `src/` (including Prisma schema at `src/prisma/`). Postgres is reached via `DATABASE_URL` in `.env`, wired in `prisma7.config.ts` (Prisma 7 style â the URL is **not** in `schema.prisma`).
...
**Gotcha you will hit:** local `DATABASE_URL` has pointed at Postgres database `template1`. Putting app tables in `template1` breaks Prisma `migrate dev`âs **shadow database** (it clones template1, already has `Tenant`, migration says âalready existsâ). SPEC-02 step 1 used `migrate deploy` with a handwritten SQL file instead. That is an environment quirk, not a product decision.

---

## Chapter 1 â planning docs and the Cursor leash

**Commit:** `9fce834` (this was `origin/main` before the feature branch).

**What we added (not product code):**

- `docs/decisions/DR-001` â tenancy, modules, transactions, notifications after commit, RLS deferred.
- `docs/decisions/DR-002` â core tables for MVP (people, venue, booking, payment, ledger, expense) and rejected alternatives.
...
**Why this before more code:** decision â spec â code. If the agent invents a table in chat, you cannot defend it. The BRD was still a placeholder here (`docs/requirements/` said âadd when readyâ).
...
## Chapter 2 â SPEC-01 Tenancy foundation (implemented)

**Goal of the slice:** prove isolation with the smallest real read: *list this tenantâs pitches*. If scoping is wrong, show **zero rows**, never Samiâs pitches on Ahmadâs site.
...
### Step 2 â Resolve tenant without touching the DB

**Files:** `src/lib/tenant-slug.ts`, `src/proxy.ts`, `test/lib/tenant-slug.test.ts`

**What:** `parseTenantSlug(host, searchParams)` â `ahmad.stadiums.com` â `"ahmad"`. Local: `ahmad.localhost`, `ahmad.lvh.me`, or `?tenant=ahmad` (query **wins** so you can test without DNS). Invalid slugs â `null`.

**`src/proxy.ts`:** Next.js 16 **proxy** (same job as old âmiddlewareâ). Runs **before** the page, in a **different runtime**. It **cannot** put tenant id in AsyncLocalStorage for the page to read (DR-001 corrected that). It only sets header `x-tenant-slug`. It **must not** query Postgres.

**Why a pure function:** Jest can test it without Next. If you ever change URL scheme, you change one function.

**Relation:** proxy â header â `tenant-context` (next step).

### Step 3 â Tenant exists? 404 if not

**File:** `src/lib/tenant-context.ts`

**What:** Page/server code reads `x-tenant-slug` via `headers()` (Next â request-time API, must `await`). Looks up `Tenant` with **`platformDb`** (unscoped â you are looking up the tenant row itself). Missing slug or unknown slug â `notFound()`. `cache()` so one lookup per request.

**Rule to defend:** *The URL chooses the tenant. A future login only asks âis this person allowed **here**?â* Invert that and a stolen cookie could show the wrong stadium.

**Relation:** `getCurrentTenantId()` is what the Prisma extension uses.

### Step 4 â The isolation guard (`db` vs `platformDb`)
...
**What `db` does:** for model `Pitch`, inject `tenantId` on reads and creates. Callers **must not** write `where: { tenantId }` themselves â that is the proof the guard works. Unique finds that return another tenantâs row are rejected.

**Why two clients:** seed and âfind tenant by slugâ **cross** tenants. If they used `db`, they would be filtered to one tenant and lie. `platformDb` is the exception hatch (DR-001).

**Why not RLS yet:** extra Postgres role and wrapping every request. Trigger: first real paying tenant who isnât a friend.

**Relation:** `listPitches()` uses `db` and never mentions `tenantId`.

### Step 5 â Thin page + venue infrastructure
...
### Step 6 â Seed two businesses

**File:** `src/prisma/seed.ts` (`npm run db:seed`). Uses an unscoped client like `platformDb`.

**What:** Wipe pitches/tenants, create `ahmad` (A1âA3) and `sami` (S1âS2). Later SPEC-02 filled `scheduleConfig` on those pitches.

**Acceptance you already ran:** two hosts (or `?tenant=`), different lists; unknown tenant 404; break the extension â empty list, not a leak.

**Jest for this slice:** only `test/lib/tenant-slug.test.ts`. Domain tests wait until there is domain logic.

---

## Chapter 3 â BRD in the repo

**File:** `docs/requirements/brd.md` (draft v0.1). Docs index and `docs/requirements/README.md` updated to point at it.

**What:** Full product vision: goals G-1â¦G-6, roles, phases, BR-1â¦BR-105, RULE-1â¦12, risks. DR-002 already cited these numbers before the file was in git.

**Relation:** SPEC-02 was written **from** BR-3/4/5/7/10/12 + DR-002 Â§2.4â2.5, not invented in chat.

---

## Chapter 4 â SPEC-02 written, then built one step at a time
...
### Step 1 â `scheduleConfig` on Pitch

**Files:** `schema.prisma` (`Json` â Postgres **jsonb**), migration `20260909030600_pitch_schedule_config`.
...
**Why default closed:** generate-slots then returns `[]` until seed overwrites â safer than `{}` which Zod would reject.

### Step 2 â Zod at the edge + money helper
...
**What:** `parseScheduleConfig(unknown)` throws on bad jsonb (`z.strictObject` â extra keys fail). Prices: strings only, two cents, `parseUsd` uses Decimal **not** `parseFloat`. Times: `HH:mm` via Zod `z.iso.time({ precision: -1 })`.

**Why:** Prisma types Json as `unknown`. Invalid owner data must not become a silent new feature. BR-40 / DR-002 Â§2.18.

**Relation:** seed and the use case both parse **before** trust. Domain receives `ScheduleConfig`, never raw Json.

### Step 3 â Pure engine

**File:** `src/modules/venue/domain/availability.ts` â `generateSlotsForDay`.

**What:** Civil date + timezone + config + occupied ranges â UTC `Date`s + Decimal prices + `available`. No `await`, no Prisma. Windows that end before they start **cross midnight** (BR-7). Full games only; leftover minutes discarded. Occupied overlap is half-open `[start, end)`.

**Why Intl for timezone:** `Date#getHours()` is the **serverâs** local time, not Beirut. We convert wall clock â UTC with `Intl` offsets.

**Relation:** Booking (later) will pass approved ranges into `occupied`. Same function. No upward import.

**Spec slip we documented:** â16:00â22:00 â four slotsâ is six hours â **six** 60-minute slots. Tests follow the **rule**, not the count typo.

### Step 4 â Jest for the engine
...
### Step 5 â Application glues DB + parse + engine

**Files:** `listPitches` also selects `scheduleConfig`; `src/modules/venue/application/get-day-availability.ts`.

**What:** `await` load pitches â parse each config (fail the request if invalid) â `occupied: []` â DTO with `16:00` local strings and `"30.00"`.
...
### Step 6 â Thin page + `?date=`

**File:** `src/app/page.tsx`

**What:** Next 16: `searchParams` is a **Promise** (`PageProps<'/'>`). `?date=` or **today in Asia/Beirut** (not UTC âtodayâ â midnight would show the wrong Lebanese day). Renders `18:00â19:00 Â· $30.00` or âClosed / No slots.â

**Relation:** still no Prisma / no `tenantId` in the page. `?tenant=` still handled in **proxy**, not in this date parser.

### Step 7 â Seed real hours

**File:** `src/prisma/seed.ts` â `parseScheduleConfig` before write.

| Pitch | Rule (so you can demo) |
|---|---|
| A1 | MonâThu 16:00â22:00 60 min $30; Fri/Sat to 23:00 **$40** |
| A2 | MonâThu 16:00â22:00 **90 min $35** |
| A3 | **Monday only** 10:00â14:00 $20 |
| S1 | 18:00â22:00 $25 (not Sunday) |
| S2 | Saturday morning 09:00â12:00 $15 |
...
## Chapter 5 â Git: finish the branch
...
## Chapter 6 â Phone on the same WiâFi

**File:** `next.config.ts` â `allowedDevOrigins: ["192.168.10.194"]`
...
**How to open:** `http://192.168.10.194:3000/?tenant=ahmad` â query `?tenant=`, not path `/tenant=ahmad`. If DHCP changes the PC IP, update the array and restart `npm run dev`.
...
| If they askâ¦ | You sayâ¦ |
|---|---|
| Why not a `slots` table? | Empty future rows forever; rule changes would desync. DR-002 Â§2.5. Engine is a function. |
...
| Why proxy not âset tenant in memoryâ? | Separate runtime from the page. Header only. |

---

## Whatâs next (do not invent)
...
## Appendix â how to append (humans and agents)
...
Do **not** delete old chapters to âclean up.â If we reversed a decision, write a short **Correction** note.

Keep entries boring and specific. This file is your memory, not a brochure.

---

## Chapter 7 â 2026-09-09 â simple file logger
...
**Why:** No shared place to record failures (invalid jsonb, seed crashes). Ops plumbing, not a DR. Node `fs` only â no Winston/Pino yet.
...
**Relation:** Shared `lib/` like `money.ts`. **Must not** import from Client Components or `src/proxy.ts` (no Node filesystem). Domain stays pure â no logger in `availability.ts`. Do not log secrets.

**How to verify:** trigger `logger.info` (e.g. `npm run db:seed`) then open `logs/<UTC-date>.log`.

---

## Chapter 8 â 2026-09-09 â SPEC-03 written (not built)

**When:** 2026-09-09

**What:** Branch `feature/spec-03-booking-public-request`. Spec for **option A**: public name+phone request â PENDING booking + person + requester participant. Exclusion SQL ships now; approve UI does not.
...
**Relation:** Booking â Venue (offered slots + price) and People. Venue must not import Booking. `user_person_links` waits for User/auth.

**How to verify:** read the spec; no product code until you OK a numbered step.

---

## Chapter 9 â 2026-09-09 â SPEC-03 step 1: Person + Booking + BookingParticipant

**When:** 2026-09-09

**What:** Three tenant-owned tables for a public PENDING request. No modules, no UI, no exclusion yet.

**Why:** DR-002 Â§2.1â2.3 (person = name+phone, unique per tenant phone), Â§2.9â2.10 (booking has no person; requester is a participant), Â§2.18 (`Decimal(12,2)`). SPEC-03 step 1. `tenant_id` on all three including the child table (DR-001 / DR-002 Â§3).

**Prisma 7 docs (then the code):**
- `enum` blocks map to Postgres enums ([Models â Defining enums](https://www.prisma.io/docs/orm/v7/prisma-schema/data-model/models)).
- Money: `Decimal` + `@db.Decimal(12, 2)` so we do not get the default `decimal(65,30)`.
- `tstzrange` has no Prisma scalar. [Unsupported field types](https://www.prisma.io/docs/orm/v7/prisma-schema/data-model/unsupported-database-features): `Unsupported("tstzrange")` creates the column; the field is **not** in Prisma Client. A required Unsupported field also drops typed `create` / `upsert` on that model (schema reference). Step 7 will insert `during` with raw SQL. We did **not** add `startAt`/`endAt` â DR-002âs column is `during`.
...
**Relation:** Tenant and Pitch gain reverse lists. Booking FK â Pitch. BookingParticipant FKs â Tenant, Booking, Person. Venue module must not import Booking (still true â we added no app code). `db.ts` still only scopes `Pitch` (step 3).
...
**How to verify:** Prisma Studio â three empty tables; Person unique on `(tenantId, phone)`; Booking has `during` tstzrange and `priceUsd` DECIMAL(12,2); no `personId` on Booking.
...
## Chapter 10 â 2026-09-09 â SPEC-03 step 2: APPROVED exclusion
...
**Why:** DR-002 Â§2.8 â the database is the double-booking guard, not app code. Prisma cannot express `EXCLUDE`; it is handwritten SQL (`.cursor/rules/200-database-prisma.mdc`). Ships now so we do not âforgetâ why we picked Postgres, even though this slice only inserts PENDING.
...
## Chapter 11 â 2026-09-09 â SPEC-03 step 3: tenant extension

**When:** 2026-09-09

**What:** `Person`, `Booking`, and `BookingParticipant` are now in `TENANT_SCOPED_MODELS`. App queries must not pass `tenantId`.

**Why:** DR-001 Â§3 â the Prisma extension is the isolation guard. SPEC-03 step 3. Child tables are scoped too, not âbecause they hang off Booking.â Prisma 7 query extensions: `$extends` + `query.$allModels.$allOperations` ([v7 query component](https://www.prisma.io/docs/orm/v7/prisma-client/client-extensions/query)).
...
## Chapter 12 â 2026-09-09 â SPEC-03 step 4: People find-or-create
...
**Why:** DR-002 Â§2.2 / RULE-8 â phone unique per stadium. SPEC-03: Booking asks People, does not own âcreate person.â DR-001: the starting use case opens `$transaction` and passes `tx`; repositories never import `db`. Prisma 7 interactive transactions: `tx` is `Prisma.TransactionClient` ([Client `$transaction`](https://www.prisma.io/docs/orm/v7/prisma-client/queries/transactions)).

**Files:**
- `src/modules/people/domain/phone.ts` â `normalizePhone` (pure)
- `src/modules/people/infrastructure/persons.ts` â `findPersonByPhone`, `createPerson` (no `tenantId` in `where`/`data`)
- `src/modules/people/application/find-or-create-person.ts`
- `test/modules/people/domain/phone.test.ts`

No `ui/` or empty folders.

**Relation:** Booking (step 7) will call `findOrCreatePerson(tx, â¦)` inside its transaction. People must not import Booking. Venue unchanged.

**How to verify:** `npm test` â new suite plus SPEC-01/02. Same digits after stripping spaces/`-`/`+` â same string. Isolation (two tenants, two people) waits until a request can write rows (step 7/8).

---

## Chapter 13 â 2026-09-09 â SPEC-03 step 5: Booking domain

**When:** 2026-09-09

**What:** Pure `resolveOfferedSlot`: the UTC window must be one of Venueâs generated slots for that civil date, and `end` must be after injected `now`. Price is copied from that slot. `overlaps` is half-open, for tests.

**Why:** SPEC-03 step 5. Booking **asks** Venue (`generateSlotsForDay`); it does not invent times or trust a posted price (DR-002 Â§2.18). `occupied` stays `[]` â PENDING must not hide the slot (RULE-2). `now` is an argument so Jest can freeze time.
...
**How to verify:** `npx jest test/modules/booking/domain/offered-slot.test.ts` â closed day and unknown window fail; a real 16:00 Wednesday slot passes with `$30.00`; `now === slot.end` fails.

### In plain language (what this step is for)

This is a **gatekeeper**. It answers one question **before** we ever touch the database:

> âIs this the kind of game Ahmadâs stadium actually sells, and has that game already finished?â

Nothing is saved yet. No person row, no booking row. Just yes/no, plus the **real** price.

**The situation:** a visitor will later click a slot on the public page, e.g. Wednesday 16:00â17:00. The form sends two times (start and end). We cannot trust that blindly. Someone can change the form and send 03:00â04:00, or a price of $1. The page is not the source of truth. The opening-hours **rule** is â the same function that draws the list: `generateSlotsForDay`.

**What `resolveOfferedSlot` does:**

1. Ask Venue: for this pitch, this calendar day, in Beirut, which games exist?
2. Is the requested start/end **exactly** one of those games? If not â reject. Closed day, wrong time, 90 minutes when games are 60 â all fail the same way (`Slot is not offered`).
3. Has that game **already ended**? If `end` is before or equal to `now` â reject (`Slot has already ended`). We pass `now` in so tests can pretend it is 5pm without waiting.
4. If both pass: return that slot **and its price from the engine**. We copy `$30.00` from Venue. We never take a price from the browser (DR-002 Â§2.18).

**What we skip on purpose:** we do **not** ask âis this already booked?â Two PENDING requests for the same hour are allowed (RULE-2). Occupied time comes later, when something is APPROVED. So this function always asks Venue with an empty occupied list.

**`overlaps`:** a small helper â do two time ranges crash into each other? 16:00â17:00 and 17:00â18:00 do **not** (the first ends when the second starts). 16:00â17:00 and 16:30â17:30 **do**. Same half-open rule as Venue. The public request does not use it yet; it is here for tests and later approve.

**Why a Booking file, not inside Venue:** Venueâs job is to **list** possible games. Bookingâs job is to **allow or refuse a request**. If we mixed them, âwhat hours do we sell?â and âmay this visitor request this hour?â would live in one place and get messy when approve and payments arrive.

**One sentence:** you may only request a real, still-open game, at the stadiumâs price â as a pure function, so we can test it without the database or the webpage.

---

## Chapter 14 â 2026-09-09 â SPEC-03 step 6: Zod for the public form

**When:** 2026-09-09

**What:** `parsePublicSlotRequest` â name, phone (8â15 digits after normalize), pitchId, start/end as UTC ISO strings. Extra keys fail.

**Why:** SPEC-03 step 6. The form is untrusted. Zod checks *shape* before the use case. Step 5 still decides âis this a real unfinished game?â Phone length lives here, not in `normalizePhone`. Zod 4: `z.strictObject`, `z.iso.datetime()` ([Zod API](https://zod.dev/api) â UTC with `Z`, no `+02:00` unless we opt in; we did not).
...
**How to verify:** `npx jest test/modules/booking/schemas/public-slot-request.test.ts` â missing name, `"12"` / `"abc"` phone, extra `priceUsd` all throw; `"03 123 456"` becomes `"03123456"`.
...
Zod never opens the database. A valid form can still be a fake slot â step 7 will run both.

---

## Chapter 15 â 2026-09-09 â SPEC-03 step 7: `requestPublicSlot`
...
**Why:** SPEC-03 step 7 / DR-001 (the starting use case owns the transaction and passes `tx`). DR-002 Â§2.10 requester is a participant. Prisma 7: `$transaction` callback + tagged `$executeRaw` for `during` ([raw SQL](https://www.prisma.io/docs/orm/v7/prisma-client/using-raw-sql); Client has no `booking.create`).
...
**How to verify:** `npm test` (33 passed). No page yet â proof write waits for step 8 + Studio. Two overlapping PENDINGs are allowed (exclusion ignores them).

### In plain language

This is the **do-it** function. Zod already checked the form. Domain already knows the rule. Now we save:

1. Load this stadiumâs pitch (wrong id / Samiâs pitch on Ahmadâs site â not found).
...
5. Insert one participant flagged âthis is who asked,â owed the full game price.
...
## Chapter 16 â 2026-09-09 â SPEC-03 step 8: thin public form

**When:** 2026-09-09

**What:** Each slot on the existing page has a name + phone form. Submit runs a Server Action: Zod â `requestPublicSlot` â redirect with `?received=1`. Slot stays listed. Plain âRequest receivedâ. No styling system.

**Why:** SPEC-03 step 8. `app/` stays thin (no Prisma, no `tenantId`). Next 16 local docs: [`forms.md`](../../node_modules/next/dist/docs/01-app/02-guides/forms.md) â `<form action={serverFn}>` still receives `FormData`. [`redirect`](../../node_modules/next/dist/docs/01-app/03-api-reference/04-functions/redirect.md) after success, **outside** try/catch. `searchParams` stays a Promise.

**Files:** `src/app/request-slot.ts`; `src/app/page.tsx`. Hidden `tenant` is the **slug** (already on the page), not `tenantId` â so local `?tenant=` survives the redirect.
...
**How to verify:**
1. `http://localhost:3000/?tenant=ahmad&date=2026-09-09` â Pitch A1 16:00 form. Submit Ali / `03 123 456`.
...
4. Same slot, different phone â second PENDING. Same phone again â same Person, new Booking.
5. `?tenant=sami` â Samiâs pitches only; Ahmadâs people/bookings stay Ahmadâs.
6. `npm test` (33).
...
The page is still just a **thin waiter**. It does not decide prices or create rows. It collects name + phone, plus hidden pitch/start/end, and hands that to the Action. The Action is the bouncer (Zod) then the kitchen (`requestPublicSlot`). After a successful save we send you back to the same dayâs list with a sticky note: âRequest received.â We do not hide the slot â PENDING is not occupancy.

---

## Chapter 17 â 2026-09-09 â Gotcha: expired interactive transaction on public request

**When:** 2026-09-09

**What:** Submitting the public form failed with Prisma âquery cannot be executed on an expired transactionâ (timeout 5000 ms, ~5029 ms elapsed) on `pitch.findUnique` â the **first** query inside `$transaction`, not a slow insert.

**Why:** `platformDb` is an alias of the same Prisma client/pool as `db` (`src/lib/prisma-base.ts`). Interactive `$transaction` holds one connection (`BEGIN`). The tenant query extension then calls `getCurrentTenantId()` â `platformDb.tenant.findUnique` on that **same** client. The Server Action is a new request, so React `cache()` is cold. Tenant lookup waits for the connection the transaction is holding; the transaction waits for the tenant lookup. After 5s Prisma kills the transaction. Prisma 7 interactive options: `timeout` / `maxWait` ([transactions](https://www.prisma.io/docs/orm/v7/prisma-client/queries/transactions)); we did **not** raise the timeout â that would only make the form hang longer.
...
Postgres will not let two things use the same âphone lineâ at once. The booking save picks up the line and says âwait, Iâm in a transaction.â The security guard then tries to look up which stadium this is â on that **same** line â and both wait until the timer rings. We look up the stadium **first** (short call, then remembered for this click), and only then start the transaction. The guardâs later checks are free memory hits, not a second database call.

---

## Chapter 18 â 2026-09-09 â Correction: `platformDb` must be a second pool

**When:** 2026-09-09

**What:** Chapter 17âs warm-`cache()` fix did **not** work. Submit still 500âd at ~5.1s on `tx.pitch.findUnique` (`run(args)` in `db.ts`). Warming tenant before `BEGIN` does not help: React `cache()` does not apply inside Prismaâs `$allOperations` callback, so the guard still queried `platformDb` **during** the transaction.

**Why:** `platformDb` was `export const platformDb = prismaBase` â one client, one pool. Prisma will not run a second query on that client while an interactive transaction is open. DR-001 already named a **separate** `platformDb`; the implementation had aliased it. Prisma 7: one `PrismaClient` per adapter/pool ([constructor](https://www.prisma.io/docs/orm/v7/prisma-client/setup-and-configuration/instantiate-prisma-client)).
...
Remembering the stadium in Reactâs notebook does not help the Prisma guard â that guard runs in a back room and never reads the notebook. Two clerks also cannot share one phone line: while the booking clerk says âIâm in a meeting,â the stadium-lookup clerk must use a **second** line. `platformDb` is that second line.

---

## Chapter 19 â 2026-09-09 â Correction: one pool, two Prisma clients

**When:** 2026-09-09

**What:** After Chapter 18, the homepage 500âd: `Connection terminated unexpectedly` on `listPitches`. Not a Venue bug â Postgres (local proxy, `connection_limit=10` on `DATABASE_URL`) dropped sockets.

**Why:** Chapter 18 gave `platformDb` its **own** `pg.Pool` (default max 10). Two pools tried to open ~20 connections against a cap of 10. HMR also left old pools alive. Prisma 7 still wants two **PrismaClient** instances so tenant lookup is not serialized onto `db.$transaction` â they must **share one Pool**.

**Files:** `src/lib/prisma-base.ts` (one `pg.Pool` `max: 10`, two clients); `src/lib/platform-db.ts` (re-export); `src/lib/db.ts` (comment).

**Relation:** Same as Chapter 18âs intent. `platformDb` is a second clerk, not a second phone company.

**How to verify:** Restart `npm run dev` (old leaked pools die with the process). `/?tenant=ahmad` renders pitches. Submit name+phone â **Request received** under 5s.

### In plain language

We gave the stadium-lookup clerk a second phone **company**, and the switchboard only has ten lines â it hung up on everyone, including the page that only lists pitches. Both clerks now share one switchboard (ten lines). They are still two different clerks, so the booking meeting does not block the stadium lookup.

---

## Chapter 20 â 2026-09-09 â Correction: one client; tenant in ALS before BEGIN
...
Two clerks sharing one switchboard still grabbed the same handset and yanked the cord out. So we went back to **one** database client. Before the booking meeting starts, we write the stadiumâs name on a sticky note on the desk. During the meeting the guard only reads the sticky note â they do not call the database again. That is allowed: DR-001 said application code (not middleware) sets request-scoped tenant context.

---

## Chapter 21 â 2026-09-09 â Guide: Prisma transaction + tenant guard

**When:** 2026-09-09

**What:** Standalone write-up of Chapters 17â20 so we can return to it without rereading the chat.
...
**How to verify:** Open that file. Â§5 is the rule; Â§4 is what not to retry.

---

## Chapter 22 â 2026-09-10 â DR-003 written (auth)

**When:** 2026-09-10

**What:** Settled Access decisions before SPEC-04. No auth code yet.

**Why:** [DR-003](./decisions/DR-003-auth-sessions.md). URL still chooses tenant; login is `local@tenant-slug` + hashed password (BR-99 vs the old âemail@domainâ note). Users have no `tenant_id`; memberships do.
...
## Chapter 23 â 2026-09-10 â SPEC-04 written (not started in code)

**When:** 2026-09-10

**What:** Numbered Access slice: schema â guard â `can`/identifier â Zod â login/logout â `/login` + `/owner` â seed `owner@ahmad` / `owner@sami`. No approve UI.
...
## Chapter 24 â 2026-09-10 â SPEC-04 step 1: Access schema
...
**Why:** SPEC-04 step 1 / DR-003 Â§3. Login accounts are not stadium rows; the **membership** is. Link table exists so we never retrofit PersonâUser (DR-002 Â§2.1). Prisma 7 schema + handwritten SQL + `migrate deploy` (local DB is `template1`; no `migrate dev` shadow).
...
**How to verify:** Prisma Studio: four new tables, `UserPersonLink` empty. `npx prisma migrate status --config prisma7.config.ts` â up to date.

### In plain language

A **User** is a login (who knows the password). A **Membership** is âthis login may work at Ahmadâs stadium as OWNER.â The cookie will remember the login, not the stadium â the URL still picks the stadium. The person-link table is a spare drawer; we do not put anything in it yet.

---

## Chapter 25 â 2026-09-10 â SPEC-04 step 2: guard vs platformDb
...
**Why:** SPEC-04 step 2 / DR-003 Â§3. A membership list must be Ahmad-only without the caller writing `tenantId`. Looking up `owner@ahmad` must not be filtered to a tenant (User has no `tenantId`). Nested `platformDb` inside `db.$transaction` is still forbidden ([guide](./guides/prisma-transaction-tenant-guard.md)).
...
The security guard now also watches **memberships** (and the empty person-link drawer). Logins and cookies are not stamped with a stadium â the guard would have nothing to stamp. We still have one database client. Access code is not written yet.

---

## Chapter 26 â 2026-09-10 â SPEC-04 step 3: Access domain
...
**Why:** SPEC-04 step 3 / DR-003 Â§2 and Â§5. Domain takes `MembershipLike`, not a Prisma type.
...
**How to verify:** `npm test` â 9 suites, 39 passed.
...
## Chapter 27 â 2026-09-10 â SPEC-04 step 4: Zod login body

**When:** 2026-09-10

**What:** `parseLogin` â `identifier` + `password`, `strictObject`, identifier run through domain normalize + shape. Password not hashed here.
...
**How to verify:** `npm test` â 10 suites, 42 passed.
...
## Chapter 28 â 2026-09-10 â SPEC-04 step 5: login / logout / membership

**When:** 2026-09-10

**What:** `login` (URL tenant â user on `platformDb` â hash verify â membership on `db` â session + HTTP-only cookie), `logout`, `getCurrentMembership`. Same `"Invalid login"` for unknown user, bad password, or no membership *here*. Cookie: no `domain` (host-only), `httpOnly`, 7 days. Next 16: `await cookies()`; `.set`/`.delete` only from a Server Action ([cookies.md](../../node_modules/next/dist/docs/01-app/03-api-reference/04-functions/cookies.md)). Password hash is `scrypt` via `node:crypto`. No `$transaction` wrapping User+membership (tenant-guard guide).
...
**Relation:** Access does not import Booking. Pages not built (step 6). No seeded owners yet (step 7) â click-test waits.

**How to verify:** `npm test` (42). Click-proof after steps 6â7.

### In plain language

The kitchen for login: first we know which stadium this URL is. Then we look up the username (not filtered by stadium). We check the password. Then we ask: does this person have a pass for **this** stadium? If not, we say âInvalid loginâ â we do not say âwrong stadium.â The cookie remembers the login, not Ahmad vs Sami. You cannot try this in the browser until we add the form and seed a password.

---

## Chapter 29 â 2026-09-10 â SPEC-04 step 6: thin login / owner pages

**When:** 2026-09-10

**What:** `/login` form (identifier + password) and `/owner` (name, identifier, role, logout). Hidden `tenant` slug for local `?tenant=`. No Prisma / no `tenantId` on pages. Invalid login â same page with âInvalid loginâ. Unauthenticated `/owner` â `/login`. Public `/` unchanged.
...
The waiter for login: a small form and a locked âyou are inâ page. They do not talk to Prisma. Until we plant a password in the database (next step), every login will fail on purpose.

---

## Chapter 30 â 2026-09-10 â SPEC-04 step 7: seed owners + staff

**When:** 2026-09-10

**What:** Seed now plants hashed logins on the unscoped client: `owner@ahmad` OWNER on Ahmad, `owner@sami` OWNER on Sami, `staff@ahmad` STAFF with `permissions: {}`. Password is `LOCAL_DEV_PASSWORD` (`dev-owner`) in `seed.ts` only â not imported from `app/`. Delete order now covers Session / Membership / UserPersonLink / User before Tenant. `npm run db:seed` ran successfully.

**Why:** SPEC-04 step 7 / DR-003 Â§2â3. Owners must exist so login can be click-tested. Staff is optional in the spec; seeded so `can(..., "bookings.approve")` stays false until a later flag.

**Files:** `src/prisma/seed.ts`; `src/app/login/actions.ts` (log unexpected login errors only â `"Invalid login"` stays silent); `docs/README.md`.

**Relation:** Seed uses its own PrismaClient (not `@/lib/db`). Access still does not import Booking. Public `/` is unchanged.

**Gotcha:** After adding User/Session models, a long-running `npm run dev` can keep an old `prismaBase` on `globalThis` (`src/lib/prisma-base.ts`). Then `platformDb.user` is undefined and login looks like âInvalid loginâ. Restart the Next process after `prisma generate` / new models. Seed in a separate process was fine; the app process was stale.

**How to verify:** `http://localhost:3000/login?tenant=ahmad` â `owner@ahmad` / `dev-owner` â `/owner` shows Ahmad Stadium. Same cookie on `?tenant=sami` â `/login`. `owner@ahmad` on Sami â Invalid login. Logout â `/owner` redirects to login. Public form still loads without a cookie. `npm test` â 10 suites, 42 passed.

### In plain language

We planted three keys in the test lockbox. Ahmadâs owner key opens Ahmadâs door. It does not open Samiâs. A staff key exists for Ahmad but cannot approve bookings yet (there is still no approve button). The password is only for local development â it is not a mailbox and must not live in the browser.

---

## Chapter 31 â 2026-09-10 â SPEC-05 written (not started in code)

**When:** 2026-09-10

**What:** Numbered owner-approve slice: `slot_interests` â guard â domain (overlap + occupied on `resolveOfferedSlot`) â Zod â infra â `approveBooking` / `rejectBooking` (auth **before** `$transaction`) â public occupied â `/owner` pending inbox. No payment, no owner-created bookings, no waitlist UI.

**Why:** [SPEC-05](./specs/SPEC-05-owner-approve.md) implements [DR-002](./decisions/DR-002-core-data-model.md) Â§2.8 / Â§2.13 and BR-17â21 / BR-23â25. Pins: auto-reject = overlapping PENDING on the same pitch; interest `during` = the **approved** window; manual reject writes no interest; same `bookings.approve` flag for reject; Venue still does not import Booking.
...
The next chapter of the product: the owner looks at a list of âplease can I have this hour?â and taps yes or no. Yes makes it a real game (the database will not allow two games on one pitch at once). Everyone else who asked for that hour is automatically told no, and we remember they were interested. The public page stops offering a Request button for that hour. We have not built that yet â only the instruction sheet.

---

## Chapter 32 â 2026-09-10 â SPEC-05 step 1: SlotInterest schema

**When:** 2026-09-10

**What:** `SlotInterest` table: `tenantId` required, `pitchId`, `during` tstzrange (Prisma `Unsupported`, same as Booking), `personId`, `createdAt`. Empty this step. No unique on (pitch, during, person) â DR-002 does not require one.

**Why:** SPEC-05 step 1 / DR-002 Â§2.13. Approval later writes âwho wanted this filled hourâ as a window, not a rejected booking id. Guard is step 2.
...
**How to verify:** Prisma Studio â `SlotInterest` (empty). Columns include `tenantId` and `during`. `npx prisma migrate status --config prisma7.config.ts` up to date after generate/deploy.

### In plain language

A spare drawer for âthis person wanted that hour.â We built the drawer. We do not put names in it until the owner taps Approve.

---

## Chapter 33 â 2026-09-10 â SPEC-05 step 2: SlotInterest on the tenant guard

**When:** 2026-09-10

**What:** `SlotInterest` added to `TENANT_SCOPED_MODELS`. Prisma `findMany`/`create` on that model get `tenantId` injected. Raw `$executeRaw` for `during` still stamps `tenantId` from ALS (same as Booking insert) â the extension does not wrap raw SQL.

**Why:** SPEC-05 step 2 / DR-001 Â§1. Interests are Ahmadâs or Samiâs, never mixed.
...
The bouncer now knows the waitlist drawer. When we later list interests, we only get this stadiumâs names, without writing `tenantId` in the kitchen.

---

## Chapter 34 â 2026-09-10 â SPEC-05 step 3: approve domain rules

**When:** 2026-09-10

**What:** `resolveOfferedSlot` takes `occupied` (default `[]`); covering APPROVED range â `"Slot is taken"`. `overlappingPendingIds` = same pitch + half-open overlap. `assertPendingForDecision`: only PENDING may be approved/rejected. No Prisma in domain.

**Why:** SPEC-05 step 3 / BR-18, BR-20 pin, BR-23 via occupied. Public request still compiles: occupied omitted â empty.
...
**How to verify:** `npm test` â new cases: adjacent 16:00â17:00 / 17:00â18:00 not overlapping; loser id returned; occupied slot throws; APPROVED status throws.
...
## Chapter 35 â 2026-09-10 â SPEC-05 step 4: Zod booking decision

**When:** 2026-09-10

**What:** `parseBookingDecision` â `bookingId` only, `strictObject`, trim + min 1. No `tenant` / `tenantId` in the schema.
...
**How to verify:** `npm test` â missing id, extra field, happy `bk_1`.
...
## Chapter 36 â 2026-09-10 â SPEC-05 step 5: Booking infrastructure

**When:** 2026-09-10

**What:** Raw SQL for `during` (`listPendingBookings`, `findBookingForDecision`, `listApprovedRanges`, `insertSlotInterest`). Prisma `updateMany` for PENDING â APPROVED/REJECTED (Client has update, not create, because Unsupported). Requester `personId` via participant find. Seed deletes `SlotInterest` before Person/Pitch.

**Why:** SPEC-05 step 5. Prisma 7 generated Booking/SlotInterest have no `create` (no `during` on the Client model). `$executeRaw` is not stamped by the extension â ALS `tenantId` in SQL, same as SPEC-03 insert.

**Files:** `src/modules/booking/infrastructure/bookings.ts`; `src/prisma/seed.ts`.

**Relation:** No use case yet. Pages unchanged.

**How to verify:** Code review. Click-proof waits for step 6â8 + a running `prisma dev`.

### In plain language

The warehouse for approve: list who asked, load one requestâs hour, mark it approved or rejected, and drop a waitlist row. We still do not have the kitchen (use case) or the waiter (buttons).

---

## Chapter 37 â 2026-09-10 â SPEC-05 step 6: approve / reject use cases

**When:** 2026-09-10

**What:** `listPendingRequests` (membership required, no `can`). `approveBooking` / `rejectBooking`: `getCurrentMembership` + `can(..., bookings.approve)` **outside** `$transaction`; then load â PENDING gate â status write. Approve auto-rejects overlapping PENDING on that pitch and inserts `slot_interests` for the **approved** window. Exclusion `23P01` / `Booking_approved_during_excl` â `"Slot no longer available"` and the whole tx rolls back.

**Why:** SPEC-05 step 6 / BR-17â21, BR-24, DR-003 (Booking asks Access; Access does not import Booking). Authorize before `$transaction` so session/User (`platformDb`) never run inside `tx` ([guides/prisma-transaction-tenant-guard.md](./guides/prisma-transaction-tenant-guard.md)).
...
**How to verify:** Code review now. Click-proof: step 8 + two PENDING on the same Ahmad hour â owner approve â one APPROVED, loser REJECTED + one interest; `staff@ahmad` gets `"Not allowed"`.
...
## Chapter 38 â 2026-09-10 â SPEC-05 step 7: occupied on the public day
...
**Why:** SPEC-05 step 7 / BR-23. Occupied = APPROVED only. Venue still does not import Booking â the page (and Bookingâs own request use case) assemble the ranges.
...
**How to verify:** After step 8, approve 16:00 on Ahmad â `/?tenant=ahmad` that day lists 16:00 with no Request; posting the old hidden start/end fails. Until then: empty APPROVED â every offered hour still has Request (same as before).

### In plain language

The public clock still shows every hour. If that hour is already a confirmed game, there is no Request button, and sneaking the old form values into a POST also fails. The slot engine still does not know what a booking is â it only receives âthis pitch is busy from A to B.â

---

## Chapter 39 â 2026-09-10 â Prisma 7 create types vs tenant stamp

**When:** 2026-09-10

**What:** `insertRequesterParticipant` still omits `tenantId` (guard stamps it). Prisma 7 `create` `data` is an Exact XOR: scalar FKs â UncheckedCreateInput (requires `tenantId`); nested `connect` â CreateInput (requires `tenant`). `$extends` does not rewrite those types. Assert the payload as the create `data` type.

**Why:** DR-001 â callers must not pass `tenantId`. Passing it would silence the error but hide whether the guard is doing its job.

**Files:** `src/modules/booking/infrastructure/bookings.ts`

**How to verify:** lints on `bookings.ts` â no `tenantId` missing error.

### In plain language

TypeScript wants us to write the stadium id on the participant row. The bouncer is supposed to write it for us. We keep omitting it and tell TypeScript âthe bouncer will fill this in.â

---

## Chapter 40 â 2026-09-10 â SPEC-05 step 8: /owner pending inbox

**When:** 2026-09-10

**What:** `/owner` lists PENDING oldest first (pitch, Asia/Beirut time, requester name + phone, requested at). Approve / Reject are `<form action={Server Action}>`. `can(..., bookings.approve)` false â list, no buttons. Actions: Zod `bookingId` â `approveBooking` / `rejectBooking`; `redirect` outside try/catch to `/owner?tenant=`. Failure â `?error=1`.

**Why:** SPEC-05 step 8 / BR-17, BR-97. Next 16 local docs: [`forms.md`](../../node_modules/next/dist/docs/01-app/02-guides/forms.md) â FormData on `action`. [`redirect`](../../node_modules/next/dist/docs/01-app/03-api-reference/04-functions/redirect.md) outside try/catch.
...
**How to verify:** One `next dev` only (do not start a second â Next already on PID shown in the lock). `prisma dev` must be listening on `DATABASE_URL` (51214). `http://localhost:3000/login?tenant=ahmad` â `owner@ahmad` / `dev-owner` â Approve. `staff@ahmad` sees the row, no buttons. `?tenant=sami` `/owner` does not list Ahmadâs PENDING.

**Gotcha:** `npm run dev --port 3001` is parsed by npm as a project folder. Extra port is `npm run dev -- -p 3001`. `Server has closed the connection` on `tenant.findUnique` is the local Prisma Postgres proxy, not Next â restart `npx prisma dev`, then kill the stale Next PID and start a single `npm run dev`.

### In plain language

The ownerâs inbox is on the same locked page as logout. Each ask is a line with Approve and Reject. Staff can look but get no buttons. The kitchen we already wrote does the real yes/no.

---

## Chapter 41 â 2026-09-10 â SPEC-05 click-proof
...
Ahmadâs owner tapped Approve in the browser and the kitchen ran. The slice is done.

---

## Chapter 42 â 2026-09-11 â Cursor/VS Code debugger
...
**Why:** Next 16 local docs: `next/dist/docs` plus [Debugging](https://nextjs.org/docs/app/guides/debugging) â `npm run dev -- --inspect` is the supported way to inspect only the process that runs app code (not every child). Not a SPEC slice.
...
**How to verify:** Stop the current `npm run dev`. Debug view â **Next.js: debug server-side** â F5. Click a gutter breakpoint in a Server Action or `src/modules/**/domain/*.ts`, hit that path in the browser (`/?tenant=ahmad`). Debugger must pause on that line. Jest: open a `test/**/*.test.ts` file â **Jest: current file** â F5.
...
## Chapter 43 â 2026-09-11 â SPEC-06 written (not started in code)
...
**Why:** [SPEC-06](./specs/SPEC-06-collect-payment.md) implements [DR-002](./decisions/DR-002-core-data-model.md) Â§2.14â2.21. Booking asks Payment; Payment writes Ledger; Payment never imports Booking. Q-2 pinned: `"payments.collect"` default deny for staff.
...
The owner can already lock an hour. Next we let him take cash for that hour: dollars, pounds, or both, at a rate he types. The notebook of âwhat the business tookâ is written in the same moment as the payment, so the two can never disagree. We have not built that yet â only the instruction sheet.

---

## Chapter 44 â 2026-09-11 â SPEC-06 step 1: Payment + ledger schema
...
**Why:** SPEC-06 step 1 / DR-002 Â§2.14â2.21. Polymorphic `sourceType + sourceId` keeps Payment closed when Expense/Shop arrive. `tenant_id` on the child tender table (DR-001). Prisma 7: `Decimal` + `@db.Decimal(p, s)` ([same as booking `priceUsd`](https://www.prisma.io/docs/orm/v7/prisma-schema/data-model/models)). No `Unsupported` â Client `create` should exist for these models.
...
**How to verify:** Prisma Studio â four empty tables; Payment has `sourceId` text, no booking FK; `npx prisma migrate status --config prisma7.config.ts` â up to date.
...
We added empty cash drawers: one for the exchange rate, one for âa payment happened,â one for the dollar/pound bits of that payment, and one for the notebook of money in/out. Nothing is collected yet. The payment row does not point at a booking column â it stores âthis is for a bookingâ plus that bookingâs id, so later a shop sale can use the same drawer without changing it.

---

## Chapter 45 â 2026-09-11 â SPEC-06 step 2: payment tables on the tenant guard
...
**Why:** SPEC-06 step 2 / DR-001 Â§1. Cash rows are Ahmadâs or Samiâs, never mixed. These models have normal `create` (no `Unsupported`), so the extension can stamp `data.tenantId` on Client create â unlike Booking inserts.
...
The bouncer now knows the cash drawers. When we later list payments, we only get this stadiumâs money, without writing `tenantId` in the kitchen.

---

## Chapter 46 â 2026-09-11 â SPEC-06 step 3: collect permission + money domain
...
**Why:** SPEC-06 step 3 / DR-003 Â§5 (jsonb flags, BR-98), DR-002 Â§2.18, RULE-4/5. Q-2 default deny for staff.
...
**How to verify:** `npm test` â 69 passed (Access collect flags, 900000 LBP at 90000 â $10, 1 LBP â $0.00, PENDING cannot collect).

### In plain language

The owner is always allowed to take cash. Staff are not, unless we later tick a box. The kitchen can now turn â900,000 pounds at 90,000â into ten dollars without using a JavaScript number, and it refuses to collect on a request that is still waiting for yes.

---

## Chapter 47 â 2026-09-11 â SPEC-06 step 4: Zod collect + rate
...
**How to verify:** `npm test` â 78 passed. `30` USD fails; `30.00` + `900000` passes; extra `tenant` fails.
...
## Chapter 48 â 2026-09-11 â SPEC-06 step 5: Payment / Ledger infrastructure

**When:** 2026-09-11

**What:** Repositories: latest/insert exchange rate; insert payment + tenders; sum collected USD by source; insert ledger IN/OUT. Booking lists APPROVED (soonest first) and loads one bookingâs status + price â no payment join. Callers omit `tenantId`; Prisma 7 create XOR asserted like participants (Chapter 39).

**Why:** SPEC-06 step 5 / DR-002 Â§2.14 (no booking_id), Â§2.21 (ledger in the same tx â use case will call both, step 6). Payment never lists bookings.
...
**How to verify:** Code review now. Click-proof waits for step 6â8. Studio: tables still empty until collect.

### In plain language

The cash drawers have clerks now: they can put a rate on the shelf, write a payment and its dollar/pound bits, add up what was already taken for a game, and write a notebook line. They still wait for the ownerâs Collect tap before any of that runs.

---

## Chapter 49 â 2026-09-11 â SPEC-06 step 6: collect / rate use cases
...
**Why:** SPEC-06 step 6 / DR-001 Â§5 (starting use case owns tx) / DR-002 Â§2.21 (ledger same tx) / [guard guide](./guides/prisma-transaction-tenant-guard.md) (no `platformDb` inside tx).
...
**How to verify:** Code review now. Click-proof: step 7â8. `npm test` â 78 passed (no new unit tests this step).
...
## Chapter 50 â 2026-09-11 â SPEC-06 step 7: /owner collect + rate

**When:** 2026-09-11

**What:** `/owner` shows current rate (OWNER can set a new one). APPROVED-with-remaining list: two-tap Collect remaining USD, plus mixed USD/LBP fields. Staff see the list, no Collect, no Set rate. Actions: Zod â use case; `redirect` outside try/catch; `?error=1` on failure. Pending inbox unchanged.

**Why:** SPEC-06 step 7 / BR-36, BR-38/39. Next 16: FormData on `action`; `redirect` outside try/catch (same as SPEC-05).

**Files:** `src/app/owner/page.tsx`, `src/app/owner/actions.ts`.

**Relation:** No Prisma / no `tenantId` on the page. Payment still does not import Booking. Seed rate is step 8 â until then âNo rate setâ; USD collect still works.

**How to verify:** After step 8 (or Set rate 90000): approve a game â Collect remaining USD â Studio payment + tender + ledger IN. `staff@ahmad` sees due row, no buttons. `?tenant=sami` does not list Ahmadâs due booking.

### In plain language

The ownerâs page now has a rate and a cash list. One button takes the rest in dollars. The mixed line is for $20 plus pounds. Staff can look; they cannot tap Collect.

---

## Chapter 51 â 2026-09-11 â SPEC-06 step 8: seed exchange rate
...
**How to verify:** `/owner?tenant=ahmad` after login shows 90000. Seed wipes bookings â request + approve again before Collect.
...
Both demo stadiums start with â90,000 pounds to the dollarâ on the shelf, so the owner can take mixed cash on the first evening without typing a rate first.

---

## Correction â collect form threw on `"30"` (2026-09-11)
...
## Chapter 52 â 2026-09-11 â SPEC-06 click-proof

**When:** 2026-09-11

**What:** After the `"30"` form correction, Collect on `/owner` succeeded. The due row turned Collected (remaining â¤ 0, omitted from the unpaid list). Kitchen ran: payment + tender(s) + ledger IN in one `$transaction`.
...
**How to verify:** Studio: that bookingâs payments, tenders with frozen `rateAtTime`, ledger IN matching USD equivalent. Second Collect on the same hour â `"Nothing due"`.
...
## Note â overpay parked (2026-09-11)
...
## Chapter 53 â 2026-09-11 â SPEC-07 written (not started in code)
...
**Why:** [SPEC-07](./specs/SPEC-07-record-expense.md) implements [DR-002](./decisions/DR-002-core-data-model.md) Â§2.22â2.23. Expense asks Payment; Payment writes Ledger; Payment never imports Expense.
...
The next instruction sheet: the owner writes âelectricity, this much cash, todayâ and the notebook records money going *out* the same way it recorded money coming *in*. We have not built that yet â only the sheet.

---

## Chapter 54 â 2026-09-11 â SPEC-07 step 1: Expense schema
...
**Why:** SPEC-07 step 1 / DR-002 Â§2.22â2.23 / DR-001 (`tenant_id` on the table). Prisma 7: `DateTime` â `TIMESTAMP(3)`, `String` â `TEXT`, enums via `CREATE TYPE` / `ALTER TYPE ... ADD VALUE` (CLI `migrate diff --from-config-datasource --to-schema`). No `Unsupported` â Client `create` should exist (unlike Booking). Local DB is `template1` â handwritten SQL + `migrate deploy`.
...
**How to verify:** Prisma Studio â `Expense` empty; Payment has `sourceId` text, no expense FK; `npx prisma migrate status --config prisma7.config.ts` â up to date.
...
We added a drawer labelled âwhat I spentâ (electricity, water, â¦) with a date and a note. There is no money column on that drawer â cash still goes through the payment drawer, with a new stamp that says âthis was an expense.â The app cannot write expenses yet; the bouncer does not know this drawer.

---

## Chapter 55 â 2026-09-11 â SPEC-07 step 2: Expense on the tenant guard
...
**Why:** SPEC-07 step 2 / DR-001 Â§1. Expense rows are Ahmadâs or Samiâs, never mixed. Expense has a normal Prisma `create` (no `Unsupported`), so the extension can stamp `data.tenantId` â same as Payment, unlike Booking inserts.
...
The bouncer now knows the âwhat I spentâ drawer. When we later list expenses, we only get this stadiumâs rows, without writing `tenantId` in the kitchen.

---

## Chapter 56 â 2026-09-11 â SPEC-07 step 3: record flag + expense date

**When:** 2026-09-11

**What:** `"expenses.record"` on Access `can` (`EXPENSES_RECORD`). Expense `domain/`: category const (no Prisma import), `occurredAtFromCivilDate` â 12:00 in the given zone via Intl offsets (not `Date#getHours`). No tender math here â Payment still owns freeze. No Zod yet (step 4).

**Why:** SPEC-07 step 3 / DR-003 Â§5 (jsonb flags, BR-98), DR-002 Â§2.22â2.23, SPEC-02 timezone idea without Expense importing Venue.
...
**How to verify:** `npm test` â 15 suites / 84 passed. OWNER can record with `{}`; STAFF default cannot; Beirut July and January both noon on that civil day.
...
Staff cannot write an expense unless the owner later ticks a flag. The kitchen now knows how to turn â15 Julyâ into a real clock time in Beirut (noon, including summer time) without asking the venue module.

---

## Chapter 57 â 2026-09-11 â SPEC-07 step 4: Zod record expense

**When:** 2026-09-11

**What:** `parseRecordExpense`: category enum, description 1â200, `occurredOn` `YYYY-MM-DD`, optional USD/LBP like collect (`"30"` â `"30.00"`). Hidden `tenant` rejected. Refine checks `isUsdString` / `isLbpString` before `parseUsd` / `parseLbp`. No use case yet (step 6).
...
**How to verify:** `npm test` â 16 suites / 90 passed.
...
## Chapter 58 â 2026-09-11 â SPEC-07 step 5: Expense / Payment / Ledger infrastructure
...
**Why:** SPEC-07 step 5 / DR-002 Â§2.14, Â§2.22 / Chapter 39 (Prisma 7 create XOR â omit `tenantId`, assert as create `data`). Local Prisma 7 Client: `create` / `findMany` + `orderBy` / `take` ([model queries](https://www.prisma.io/docs/orm/v7/prisma-client/queries/crud)).
...
**How to verify:** Code review now. Studio can still insert Expense by hand. Click-proof: step 7â8. `npm test` â 90 passed (no new unit tests this step).

### In plain language

The drawers can store an expense note and list the last twenty. Cash-out can now be stamped âthis was an expenseâ and dated the day the owner typed, not only âright now.â There is still no Record button on the page.

---

## Chapter 59 â 2026-09-11 â SPEC-07 step 6: record / list expense use cases

**When:** 2026-09-11

**What:** `recordExpense` authorizes `"expenses.record"` **before** `$transaction`, then date â insert Expense â freeze tenders â `recordPayment(OUT, EXPENSE, occurredAt)` in the **same** `tx`. `listRecentExpenses` requires membership only; attaches USD via Payment sums (no payment join). No Server Actions yet (step 7).

**Why:** SPEC-07 step 6 / DR-001 Â§5 (starting use case owns tx) / DR-002 Â§2.21â2.22 / [guard guide](./guides/prisma-transaction-tenant-guard.md) (no `platformDb` inside tx). Prisma 7 interactive `$transaction` ([Client `$transaction`](https://www.prisma.io/docs/orm/v7/prisma-client/queries/transactions)).
...
**How to verify:** Code review now. Click-proof: step 7â8. `npm test` â 90 passed (no new unit tests this step).

### In plain language

The kitchen can now write âelectricity, this cash, that dayâ: check the owner is allowed, freeze pounds to dollars, write the note and the notebook out-row in one meeting. There is still no Record button on the page.

---

## Chapter 60 â 2026-09-11 â SPEC-07 step 7: /owner expense form

**When:** 2026-09-11

**What:** `/owner` Expenses: OWNER (and staff with the flag) get category, description, date (default today Beirut), USD + LBP, Record. Recent 20 listed for any logged-in member. Staff seed: list, no form. Pending / rate / collect unchanged. Action: Zod â use case; `redirect` outside try/catch; `?error=1` on failure.

**Why:** SPEC-07 step 7 / BR-50â52. Next 16 local docs: [`forms.md`](../../node_modules/next/dist/docs/01-app/02-guides/forms.md) â FormData on `action`. [`redirect`](../../node_modules/next/dist/docs/01-app/03-api-reference/04-functions/redirect.md) outside try/catch (same as SPEC-05/06).

**Files:** `src/app/owner/page.tsx`, `src/app/owner/actions.ts`.

**Relation:** No Prisma / no `tenantId` on the page. Payment still does not import Expense. Seed wipe of Expense is step 8.

**How to verify:** `owner@ahmad` / `dev-owner` â Record electricity + `1800000` LBP â list shows ~$20.00. Studio: expense + payment EXPENSE + LBP tender + ledger OUT. `staff@ahmad` sees list, no Record. No browser tools in this session â click-proof is yours.
...
The ownerâs page now has a spend form. Electricity + pounds today, tap Record, it should show on the list. Staff can look; they cannot tap Record.

---

## Chapter 61 â 2026-09-11 â SPEC-07 step 8: seed wipe Expense
...
**How to verify:** `/owner?tenant=ahmad` after login shows 90000 and âNo expenses yet.â Seed wipes bookings â request + approve again before Collect.
...
## Correction â `/owner` `tx.expense` undefined (2026-09-11)
...
**Files:** `src/lib/prisma-base.ts` â if the cached client has no `expense.findMany`, disconnect and create a new one.

**How it connects:** Expense infra is unchanged. Restart `npm run dev` if HMR still holds an old `$extends` wrapper.

**How to verify:** Reload `/owner?tenant=ahmad` â âNo expenses yet.â, not a TypeError.

---

## Chapter 62 â 2026-09-11 â SPEC-07 click-proof
...
## Chapter 63 â 2026-09-11 â SPEC-08 written (not started in code)
...
**Why:** [SPEC-08](./specs/SPEC-08-financial-dashboard.md) implements [DR-002](./decisions/DR-002-core-data-model.md) Â§2.20â2.21 / BR-54â56, BR-59. BR-57 (games/pitch) and BR-58 (outstanding as a fourth number) stay out â due list already exists.
...
The next instruction sheet: three numbers at the top of the owner page â cash in, money out, whatâs left â for this month or any dates he picks. The notebook is already being written; this sheet is only how to add those numbers up. We had not built that yet when this chapter was written.

---

## Chapter 64 â 2026-09-11 â SPEC-08 step 1: ledger period index
...
**Why:** SPEC-08 step 1 / BR-59 / DR-002 Â§2.20. Period SUM filters `occurredAt`; a composite index keeps that query cheap on the droplet. Prisma 7 `migrate diff --from-config-datasource --to-schema` produced `CREATE INDEX "LedgerEntry_tenantId_occurredAt_idx"`.
...
**How to verify:** `npx prisma migrate status --config prisma7.config.ts` â up to date. Prisma Studio â `LedgerEntry` columns unchanged.
...
## Chapter 65 â 2026-09-11 â SPEC-08 step 2: reports.view + period domain

**When:** 2026-09-11

**What:** `"reports.view"` (`REPORTS_VIEW`) on `can()`. Ledger domain: civil From/To â `[start, end)` at Beirut midnight; current calendar month; `netUsd`; LBP display multiply ROUND_HALF_UP to integer pounds. No Prisma. No Expense/Venue/Payment imports.

**Why:** SPEC-08 step 2 / DR-003 (OWNER always; STAFF default deny) / DR-002 Â§2.20â2.21 (period on `occurredAt`; LBP never stored). Inclusive start / exclusive next-day midnight so a July month does not pick up 1 Aug 00:00 Beirut.
...
**How to verify:** `npm test` â 18 suites / 103 tests. Summer July 2026 midnight Beirut; winter one-day range; `20.00 Ã 90000` â `1800000`; STAFF without the flag cannot view reports.

### In plain language

Staff still cannot see the money page unless we later tick a box. The rules for âthis month in Beirutâ and âin minus outâ now exist as plain functions. The screen still does not add anything up.

---

## Chapter 66 â 2026-09-11 â SPEC-08 step 3: Zod period query

**When:** 2026-09-11

**What:** GET query schema: optional `from`/`to` (`YYYY-MM-DD`, real calendar days, `from <= to`, both together or both omitted), `view` `usd`|`lbp` (default usd), `displayRate` emptyâomit else positive LBP integer. `strictObject` â `tenant` is not in this schema. `parseLbp` only after `isLbpString`.
...
**How to verify:** `npm test` â 19 suites / 110 tests. Happy July 2026; `from` after `to` throws; `2026-02-31` throws; `view=lbp` + `displayRate=90000`; empty rate omitted; extra `tenant` rejected.

### In plain language

The date form on the owner page now has a checklist for âthis is a real from/to, and LBP view has a whole-pound rate.â Nothing is summed yet.

---

## Chapter 67 â 2026-09-11 â SPEC-08 step 4: SUM ledger by direction

**When:** 2026-09-11

**What:** `sumAmountUsdByDirection(tx, start, end)` â Prisma 7 `groupBy` `by: ["direction"]`, `_sum.amountUsd`, `occurredAt` in `[start, end)`. Missing direction â `0.00`. Convert Prisma Decimal via `.toString()` into decimal.js. No `tenantId` argument. No `sourceType` filter.

**Why:** SPEC-08 step 4 / DR-002 Â§2.20. Guard already injects `tenantId` on `groupBy` (`src/lib/db.ts`). Local generated client: `LedgerEntry.groupBy` + `_sum` (`src/app/generated/prisma/models/LedgerEntry.ts`).
...
**How to verify:** Prisma Studio â pick two `LedgerEntry` rows (IN and OUT) in a known range; the function should return those sums for that tenant only. `npm test` still 19 / 110.

### In plain language

The notebook can now be asked: âhow much came in and how much went out between these two instants?â The owner page still does not ask that question.

---

## Chapter 68 â 2026-09-11 â SPEC-08 step 5: summarizeLedgerPeriod

**When:** 2026-09-11

**What:** `summarizeLedgerPeriod({ from?, to? })` â membership + `"reports.view"` before any query; missing dates â current Beirut month; domain bounds; infra SUM; `netUsd`. Returns `{ inUsd, outUsd, netUsd, from, to }`. No `$transaction`. No Payment import (rate stays on the page).

**Why:** SPEC-08 step 5 / DR-003 (`can` before work) / DR-002 Â§2.20 (ledger is the read). Staff seed cannot (flag omitted).
...
**How to verify:** `npm test` â 19 / 110. Owner path is click-proof in step 6; staff without the flag get `"Not allowed"` if the use case is called.
...
## Chapter 69 â 2026-09-11 â SPEC-08 step 6: /owner This period

**When:** 2026-09-11

**What:** `/owner` shows In / Out / Difference for the resolved period (default current Beirut month). GET form: From, To, View USD/LBP, optional display rate, hidden `tenant`. Zod failure â default month + USD (not `?error=1`). LBP is `usdToDisplayLbp` using typed rate else current stored rate. Staff without `"reports.view"`: block omitted. No Prisma / no `tenantId` on the page. No Server Action.

**Why:** SPEC-08 step 6 / BR-54â56. Next 16 `searchParams` is a Promise (`node_modules/next/dist/docs/01-app/03-api-reference/03-file-conventions/page.md`). Native `<form method="get" action="/owner">`.
...
**How to verify:** Log in `owner@ahmad` â `/owner?tenant=ahmad`. Top of page: This period with In/Out/Difference. Change dates; empty day â `$0.00`. View LBP at 90000. `staff@ahmad`: no This period. `npm test` â 19 / 110.
...
## Chapter 70 â 2026-09-11 â SPEC-08 click-proof
...
**Why:** SPEC-08 whole-slice acceptance / DR-002 Â§2.20 (dashboard reads ledger only).
...
**How to verify:** `/owner?tenant=ahmad` as owner â three numbers; `staff@ahmad` â no This period.

### In plain language

The owner can see this monthâs cash in, money out, and the difference without opening Studio. This slice is done.

---

## Chapter 71 â 2026-09-11 â SPEC-09 written (not started in code)
...
The next instruction sheet: the owner is on the phone, taps a free hour, types who called, and the hour is taken â he does not approve himself. We have not built that yet â only the sheet.

---

## Chapter 72 â 2026-09-11 â SPEC-09 step 1: bookings.create flag
...
**How to verify:** `npm test` â 19 suites / 114 tests.

### In plain language

Staff still cannot book a callerâs hour unless we later tick a box. The owner page still has no Book button.

---

## Chapter 73 â 2026-09-11 â SPEC-09 step 2: Zod owner Book form

**When:** 2026-09-11

**What:** `parseOwnerCreateBooking`: name, phone (8â15 digits after normalize), pitchId, UTC `start`/`end`. `strictObject` â no price, no `tenant`. Same phone helper as public request.

**Why:** SPEC-09 step 2. Price stays a Venue snapshot at insert time (DR-002 Â§2.18). Hidden tenant is not isolation.
...
**How to verify:** `npm test` â 20 suites / 118 tests. Happy `03 123 456` â `03123456`; `priceUsd` extra key throws.

### In plain language

The Book form now has a checklist for âwho called and which hour.â Nothing is written to the database yet.

---

## Chapter 74 â 2026-09-11 â SPEC-09 step 3: insert APPROVED OWNER

**When:** 2026-09-11

**What:** `insertApprovedOwnerBooking` â `$executeRaw` `tstzrange` `[)`, `status = APPROVED`, `source = OWNER`. `tenantId` from ALS (extension does not stamp raw SQL). Reuse `insertRequesterParticipant` (no new participant helper). No Payment import.

**Why:** SPEC-09 step 3 / DR-002 Â§2.8. Client has no `booking.create` (Unsupported `during`). Exclusion applies immediately because the row is APPROVED.

**Files:** `src/modules/booking/infrastructure/bookings.ts`.

**Relation:** Public PENDING insert unchanged. Use case (step 4) will call this inside `$transaction`. `/owner` still has no Book form.

**How to verify:** `npm test` still 20 / 118. Click-proof in step 5: Studio OWNER + APPROVED; second overlapping APPROVED â exclusion.

### In plain language

We can now write âthis hour is taken, the owner booked itâ into the database the same way we write a public request â except it is already confirmed. The owner page still cannot do that.

---

## Chapter 75 â 2026-09-11 â SPEC-09 step 4: createOwnerBooking

**When:** 2026-09-11

**What:** `createOwnerBooking` â `"bookings.create"` before `$transaction`; `resolveOfferedSlot` + find-or-create person; insert APPROVED OWNER + requester; reject overlapping PENDING + interests. Exclusion â `"Slot no longer available"`. No Payment. Extracted `rejectOverlappingPending` and `isExclusionViolation` so Approve uses the same kitchen.

**Why:** SPEC-09 step 4 / BR-14 / DR-002 Â§2.8 / SPEC-03 transaction guide (auth before tx).
...
**How to verify:** `npm test` â 20 / 118. Staff seed cannot create (`"Not allowed"`). Click-proof in step 5.
...
## Chapter 76 â 2026-09-11 â SPEC-09 step 5: /owner Book a slot

**When:** 2026-09-11

**What:** `/owner` Book a slot if `"bookings.create"`: GET `bookOn` (default today Beirut) lists computed slots via `getDayAvailability` + `listApprovedOccupied`. Available hours: name + phone â `submitCreateOwnerBooking`. Taken hours: time only. Staff: no section. No Prisma / no `tenantId`. No cancel buttons.

**Why:** SPEC-09 step 5 / BR-13â14. Next 16: `searchParams` Promise (`page.js`); `<form action={Server Action}>` + `redirect` outside try/catch. Occupied is an argument â Venue does not import Booking.

**Files:** `src/app/owner/page.tsx`; `src/app/owner/actions.ts`.

**Relation:** Collect stays a second tap on Due bookings. This period / pending / expenses unchanged.

**How to verify:** `npm test` â 20 / 118. Log in `owner@ahmad` â Book a free hour; Studio APPROVED OWNER; public that hour occupied; `staff@ahmad` no Book form.
...
## Chapter 77 â 2026-09-11 â SPEC-09 click-proof
...
**Why:** SPEC-09 whole-slice acceptance / BR-13â14.
...
## Chapter 78 â 2026-09-11 â SPEC-10 written (not started in code)

**When:** 2026-09-11

**What:** Fast-forward `main` `40e3ece` â `5d5c77b` (SPEC-09). Branched `feature/spec-10-cancel-booking`. Wrote numbered cancel slice: owner (or `"bookings.cancel"`) flips **APPROVED â CANCELLED**. No refund / ledger write. No hours-before-kickoff check (BR-26). Waitlist UI / WhatsApp / player cancel / no-show / `tenant.settings` out. Confirmed list shows **all** APPROVED (including paid) so Cancel is reachable after Collect.

**Why:** [SPEC-10](./specs/SPEC-10-cancel-booking.md) implements BR-26 / RULE-1. Exclusion already ignores non-APPROVED (DR-002 Â§2.8). Ledger stays append-only â cash IN is not reversed this slice (DR-002 Â§2.21). `"bookings.cancel"` default deny, not reused from create/approve.
...
The next instruction sheet: the owner taps Cancel on a confirmed game and the hour is free again. Money already collected is left as-is until a later refund slice. We have not built that yet â only the sheet.

---

## Chapter 79 â 2026-09-11 â SPEC-10 step 1: bookings.cancel flag
...
**How to verify:** `npm test` â 20 suites / 122 tests.
...
## Chapter 80 â 2026-09-12 â SPEC-10 step 2: assertApprovedForCancel

**When:** 2026-09-12

**What:** `assertApprovedForCancel` â only `APPROVED` may be cancelled. PENDING / REJECTED / CANCELLED / NO_SHOW â `"Only a confirmed booking can be cancelled"`. Pure domain, no Prisma.
...
**How to verify:** `npm test` â 20 suites / 127 tests.

### In plain language

The kitchen now knows âyou can only cancel a confirmed game.â Nothing is written to the database yet.

---

## Chapter 81 â 2026-09-12 â SPEC-10 step 3: setApprovedCancelled

**When:** 2026-09-12

**What:** `setApprovedCancelled` â `booking.updateMany` where `id` + `status = APPROVED` â `CANCELLED`. `count !== 1` â `"Booking not found"`. No `tenantId` argument (guard). Not raw SQL: generated Client still has `updateMany` on Booking; `during` is Unsupported, status is not (same as `setPendingStatus`).

**Why:** SPEC-10 step 3 / DR-002 Â§2.8. CANCELLED drops out of exclusion without touching Payment or Ledger.

**Files:** `src/modules/booking/infrastructure/bookings.ts`.

**Relation:** Use case (step 4) will call this inside `$transaction`. `/owner` still has no Cancel.

**How to verify:** `npm test` â 20 / 127. Click-proof in step 5: Studio CANCELLED; a PENDING id fails.
...
## Chapter 82 â 2026-09-12 â SPEC-10 step 4: cancelBooking

**When:** 2026-09-12

**What:** `cancelBooking` â `"bookings.cancel"` **before** `$transaction`; load via `findBookingForDecision`; `assertApprovedForCancel`; `setApprovedCancelled`. Log after commit. No Payment / Ledger / WhatsApp.
...
**How to verify:** `npm test` â 20 / 127. Staff seed cannot cancel (`"Not allowed"`). Click-proof in step 5.
...
## Chapter 83 â 2026-09-12 â SPEC-10 step 5: /owner Cancel

**When:** 2026-09-12

**What:** `/owner` **Confirmed bookings** = all APPROVED (paid included). Collect only if remaining > 0 and `payments.collect`. Cancel if `bookings.cancel`. Server Action: reuse `parseBookingDecision` â `cancelBooking` â `redirect` outside try/catch (Next redirect.md). Preserve `tenant` + `bookOn`. No Prisma / no `tenantId`. No waitlist / refund fields.

Local Next docs: `forms.md` â `<form action>` receives FormData; `redirect.md` â `redirect` outside try/catch.
...
**How to verify:** `npm test` â 20 / 127. Log in `owner@ahmad` â Cancel a confirmed hour; Studio CANCELLED; public that hour requestable; `staff@ahmad` no Cancel. Could not click in a browser from this session (no browser tools); GET `/owner?tenant=ahmad` compiled (307 to login).
...
## Chapter 84 â 2026-09-12 â SPEC-10 click-proof
...
## Chapter 85 â 2026-09-12 â SPEC-11 written (not started in code)

**When:** 2026-09-12

**What:** Wrote numbered waitlist slice: `/owner` lists people with `slot_interests` on a **free, not-ended** window; **Notify** is `wa.me` + prefilled English âslot availableâ (BR-30 / BR-69). No Cloud API, no interest delete, no other BR-71 templates. Stayed on `feature/spec-10-cancel-booking` because SPEC-10 is still uncommitted (cannot FF-merge `main`).

**Why:** [SPEC-11](./specs/SPEC-11-waitlist.md) implements BR-29 / BR-30 / DR-002 Â§2.13. Interests already written on auto-reject (SPEC-05). Notification module owns `wa.me` (DR-001); Booking never formats WhatsApp. No new `can()` flag â listing is not a mutation.
...
The next instruction sheet: after a game is cancelled, the owner sees who wanted that hour and can open WhatsApp with a ready-made message. We have not built that yet â only the sheet.

---

## Chapter 86 â 2026-09-12 â SPEC-11 step 1: WhatsApp link + message

**When:** 2026-09-12

**What:** `whatsAppHref` â Lebanon digits to `https://wa.me/<e164>?text=` (`03â¦` â `961â¦`; already-`961` unchanged). Empty/spaces/too-short â `"Phone cannot be used for WhatsApp"`. `slotAvailableMessage` English: stadium, pitch, local startâend. No send. Notification does not import Booking.
...
**How to verify:** `npm test` â 21 suites / 136 tests.
...
## Chapter 87 â 2026-09-12 â SPEC-11 step 2: isWaitlistWindowOpen

**When:** 2026-09-12

**What:** `isWaitlistWindowOpen` â window is open if `end > now` and no occupied range on that pitch overlaps (reuse `overlaps()`). Caller passes APPROVED only; CANCELLED is omitted so the hour is open.

**Why:** SPEC-11 step 2 / BR-29 / DR-002 Â§2.13. Occupied is an argument â domain does not read Prisma.
...
**How to verify:** `npm test` â 22 suites / 140 tests.

### In plain language

The kitchen now knows âthis hour is free again and not in the past.â Nothing is loaded from the database yet.

---

## Chapter 88 â 2026-09-12 â SPEC-11 step 3: listSlotInterestsWithPeople

**When:** 2026-09-12

**What:** `listSlotInterestsWithPeople` â `$queryRaw` join Pitch + Person, `lower`/`upper(during)`, `tenantId` from ALS (extension does not stamp raw SQL). No `tenantId` argument. No `SlotInterest.create` (`during` still Unsupported). Open vs occupied stays domain.

**Why:** SPEC-11 step 3 / DR-002 Â§2.13. Same raw pattern as Booking lists.

**Files:** `src/modules/booking/infrastructure/bookings.ts`.

**Relation:** Use case (step 4) will filter open windows and attach `wa.me`. `/owner` still has no Waitlist.

**How to verify:** `npm test` â 22 / 140. Click-proof in step 5: Ahmad rows only when ALS is Ahmad.

### In plain language

We can now load âwho wanted which hourâ for this stadium. The owner page still has no Waitlist.

---

## Chapter 89 â 2026-09-12 â SPEC-11 step 4: listOpenWaitlist

**When:** 2026-09-12

**What:** `listOpenWaitlist` â membership else `"Not allowed"` (no extra `can()`); interests + `listApprovedRanges`; `isWaitlistWindowOpen`; dedupe person per window; group soonest first. `wa.me` from Notification + tenant name (`getCurrentTenant`). Bad phone â no href, still listed. No `$transaction`. No Payment. Does not log phones.
...
**How to verify:** `npm test` â 22 / 140. Click-proof in step 5: after cancel of an hour with a loser, groups appear; still-APPROVED hours do not.

### In plain language

The kitchen can now assemble âthese people wanted this free hour, here is the WhatsApp link.â The owner page still has no Waitlist.

---

## Chapter 90 â 2026-09-12 â SPEC-11 step 5: /owner Waitlist

**When:** 2026-09-12

**What:** `/owner` **Waitlist** from `listOpenWaitlist`. Group: pitch + local range; people name + phone; **Notify** is `<a href={wa.me}>` (`target="_blank"` `rel="noopener noreferrer"`), not a Server Action. Empty: âNo waitlist.â No Prisma / no `tenantId`. Public page unchanged.

Local Next docs: `page.md` â Server Component, `searchParams` Promise. GET render; no `redirect` for Notify.
...
**How to verify:** `npm test` â 22 / 140. Two public requests same hour â approve one â cancel â Waitlist shows the loser; Notify is `wa.me`. Could not click from this session (no browser tools); GET `/owner?tenant=ahmad` compiled (307 to login).
...
## Chapter 91 â 2026-09-12 â SPEC-11 click-proof

**When:** 2026-09-12

**What:** Three public requests same hour (`feda75a4-â¦`, `66c81d4a-â¦`, `7a491948-â¦`). Approve the first; the other two became REJECTED + SlotInterest (`p2` / `71234234`, `p3` / `71345345`). Waitlist empty while APPROVED. Cancel `feda75a4-07ed-46fc-a1c0-893dc05beb8e` â Waitlist showed those people. Notify is `wa.me` (no new DB row).
...
**How to verify:** Studio those interest ids on pitch `cmtw9vxs80001cwl2ufvexnjy`, window 13:00â14:00Z; booking `feda75a4-â¦` is `CANCELLED`.
...
## Chapter 92 â 2026-09-12 â SPEC-12 written (not started in code)

**When:** 2026-09-12

**What:** Branched `feature/spec-12-error-handling` from `main`. Wrote DR-004 + numbered error-handling slice: `DomainError` / `UnexpectedError`, English dictionary by key, logger context, actions never throw uncaught (except `redirect`), `?error=<key>` banners, Next `error.tsx` (`retry`, not `reset`). Public request currently has no try/catch â that is the crash. No next-intl, no `useActionState` rewrite, no wrap-every-Prisma.
...
The next instruction sheet: you still get everything in the log file; the owner gets âthat hour has already ended,â not a broken screen. We have not built that yet â only the sheet.

---

## Chapter 93 â 2026-09-12 â SPEC-12 step 1: DomainError + dictionary

**When:** 2026-09-12

**What:** `DomainError` carries a message key. `UnexpectedError` wraps `unknown` as `cause`. English catalog + `errorMessage(key)` (unknown / legacy `1` â generic). No UI, no action changes.
...
**Relation:** Existing `throw new Error("â¦")` still in place (step 4). Logger unchanged (step 2).

**How to verify:** `npm test` â 23 suites / 144 tests.

### In plain language

We now have two kinds of error in code: âthis can happenâ (a key) and âthis is a bugâ (keep the original). Nothing on the ownerâs screen has changed yet.

---

## Chapter 94 â 2026-09-12 â SPEC-12 step 2: logger context
...
**How to verify:** `npm test` â 24 suites / 146 tests.
...
## Chapter 95 â 2026-09-12 â SPEC-12 step 3: error.tsx + global-error.tsx

**When:** 2026-09-12

**What:** `error.tsx` and `global-error.tsx` â Client Components. Local Next error.md: **`retry`**, not `reset`. `global-error` includes `<html>` + `<body>`. English + Arabic retry. Digest in `<code>` if present. No `error.message`, no logger, no Prisma.
...
**How to verify:** `npm test` â 24 / 146. JSX has no `error.message`.

### In plain language

If the page blows up, the owner now sees âtry againâ in English and Arabic, not a stack. Forms can still crash until later steps.

---

## Chapter 96 â 2026-09-12 â SPEC-12 step 4: DomainError at throw sites

**When:** 2026-09-12

**What:** Domain + known infra product throws now use `DomainError("â¦key")`. `.message` is the key, not English. `lib/money.ts` and `lib/db.ts` tenant-scope throws left as-is. Application use cases still match English `EXPECTED` sets (step 5).
...
**How to verify:** `npm test` â 24 suites / 146 tests. Grep `src/modules/**/domain` and `infrastructure` for `throw new Error` â none.

### In plain language

The kitchen now throws a code like âthat hour ended,â not a sentence. The owner still sees the old banners until we teach the front desk (use cases and forms) those codes.

---

## Chapter 97 â 2026-09-12 â SPEC-12 step 5: use cases wrap unexpected

**When:** 2026-09-12

**What:** Mutating use cases rethrow `DomainError` (no error log). Exclusion â `booking.slot_unavailable` (error-level collision log). Else `logger.error` with `useCase` + `safeTenantId()` (outside `$transaction`) and `UnexpectedError`. List/read `Not allowed` â `access.not_allowed`. Login invalid â `access.invalid_login`. Actions still match English (step 6).
...
**How to verify:** `npm test` â 24 / 146. Grep `src/modules/**/application` for `throw error` / `throw new Error` â none. Catch-alls call `rethrowUnexpected`.

### In plain language

The kitchen now says âthis is a normal noâ vs âthis is a bug â write it in the log with the stadium name.â The forms still show a generic âerror=1â until the next step.

---

## Chapter 98 â 2026-09-12 â SPEC-12 step 6: action banners (`?error=<key>`)

**When:** 2026-09-12

**What:** Owner, login, and public Server Actions catch (except `redirect` â local redirect.md: redirect throws) â `?error=<key>`. `DomainError` â its key; Zod â `form.invalid`; unexpected â `error.generic` (already logged in the use case). Pages print `errorMessage(key)`. Public Request now has try/catch. Legacy `error=1` still maps to generic.
...
**How to verify:** `npm test` â 25 / 149. Public Request on 2026-09-11 16:00 Ahmad Pitch A1 â 303 `/?tenant=ahmad&date=2026-09-11&error=booking.slot_ended` and the page shows âThat hour has already ended.â Bad login â `error=access.invalid_login`. No crash overlay.
...
## Chapter 100 â 2026-09-12 â visual system tokens + owner Login
...
**How to verify:** `http://localhost:3000/login?tenant=ahmad` â Card, Volt Log in button, Plex/Kufi/Mono on `<html>`. `?error=access.invalid_login` shows âInvalid login.â CSS `--primary: #10b981`. Owner schedule and public booking pages not restyled.
...
## Chapter 101 â 2026-09-12 â Login card actually centered

**When:** 2026-09-12

**What:** Login was a full-width strip stuck to the top (`min-h-full` never filled the viewport). Main is now `min-h-svh` + column + center. Card `max-w-sm`. Title is âLog inâ; stadium name stays tenant-authored. Same tokens, same `submitLogin`.
...
**How to verify:** `/login?tenant=ahmad` â narrow card in the middle of the screen, not a banner at the top.
...
## Chapter 102 â 2026-09-12 â owner Today + restyle `/owner`
...
The owner phone screen now starts with âwho is waitingâ and âwho is on the pitch,â in the same colors as login. The kitchen still does the same jobs; it just stopped looking like a notepad.

---

## Chapter 103 â 2026-09-12 â custom Select and Calendar on owner
...
**Why:** Visual system â OS date pickers and dropdowns broke the Carbon/Volt look. Not a SPEC. Actions and Prisma stay put.
...
The owner forms no longer pop up the phoneâs own calendar and menu. They use our green-ring picker and list instead, but they still send the same dates and category codes to the kitchen.

---

## Chapter 104 â 2026-09-12 â public booking restyle
...
**Why:** Visual system â public was still `system-ui`. Calendar plan said reuse DateField for `?date=`. Not a SPEC.
...
**How to verify:** `/?tenant=ahmad` â heading, calendar Day, Show slots, Request on a free hour. `?date=2026-09-13` still lists that civil day. Success still âRequest receivedâ; `?error=` still the dictionary line.
...
**How to verify:** `/?tenant=ahmad` â Hours, not a duplicate âSchedule forâ line; Request then a toast âRequest received.â `/login?tenant=ahmad` â in-card error, Log in pending. `/owner` after login â Pending Â· n, empty blocks, Approve toast, Collect still Volt, mixed secondary. Title âStadiumsâ.
...
## Chapter 106 â 2026-09-12 â Scoped Suspense (keep GET, no full-page skeleton)
...
**How to verify:** `/?tenant=ahmad` â Day + Show hours stay; Hours list skeletons then slots. `/login?tenant=ahmad` â login card only (no hours skeleton). `/owner` after login â header + Book day stay; Today / slots skeleton then fill; Approve keeps the header.
...
## Chapter 107 â 2026-09-12 â SPEC-13 written (not started in code)

**When:** 2026-09-12

**What:** Product chose Arabic/RTL next. Wrote DR-005 + numbered SPEC-13: `dir="rtl"` `lang="ar"`, Arabic dictionaries for the existing error/success keys plus chrome `ui()`, WhatsApp âslot availableâ body in Arabic. No next-intl, no `/ar` URL, no language switch, no `tenant.settings`. Calendar stays `en-GB` (Western digits). Tenant-authored names stay as written.
...
## Chapter 108 â 2026-09-12 â SPEC-13 step 1: Arabic dictionaries
...
**Why:** SPEC-13 step 1 / DR-005. Copy is not in JSX yet so step 3â4 only swap callers.
...
**How to verify:** `npm test` â 27 suites, 154 passed. UI still English until later steps; toasts would already be Arabic if you trigger `ok=` / `error=`.
...
## Chapter 109 â 2026-09-12 â SPEC-13 step 2: dir + crash screens

**When:** 2026-09-12

**What:** Root `<html lang="ar" dir="rtl">`. Title `ÙÙØ§Ø¹Ø¨` via `ui("doc.title")`. `error.tsx` / `global-error` are Arabic-first with English as a second `dir="ltr"` line; retry button `Ø­Ø§ÙÙ ÙØ±Ø© Ø£Ø®Ø±Ù / Try again`. `global-error` does not import `ui-copy`. Grep of `src/` found no `pl-`/`pr-`/`ml-`/`mr-`/`text-left`/`text-right`/`left-0`/`right-0` (Radix `data-[side=left]` animations left as-is).
...
**How to verify:** View source of `/?tenant=ahmad` â `lang="ar"` `dir="rtl"` `<title>ÙÙØ§Ø¹Ø¨</title>`. Login labels still English.
...
## Chapter 110 â 2026-09-12 â SPEC-13 step 3: public + login chrome
...
**How to verify:** `/?tenant=ahmad` â Ø§ÙÙÙÙ / Ø¹Ø±Ø¶ Ø§ÙØ³Ø§Ø¹Ø§Øª / Ø§ÙØ³Ø§Ø¹Ø§Øª / Ø§Ø·ÙØ¨; pitch names unchanged. `/login?tenant=ahmad` â ØªØ³Ø¬ÙÙ Ø§ÙØ¯Ø®ÙÙ / Ø¯Ø®ÙÙ. Bad password still in-card via `access.invalid_login`.

### In plain language

The visitor and the lock on the door now speak Arabic. The ownerâs kitchen list is still English until the next step.

---

## Chapter 111 â 2026-09-12 â SPEC-13 step 4: owner chrome

**When:** 2026-09-12

**What:** Owner header, Today, Book, Waitlist, period, rate, and expenses use `ui(...)` / count and money helpers. `categoryLabel` reads `cat.*`. Role is `role.OWNER` / `role.STAFF`. GET names (`bookOn`, `from`, `to`, `view`) unchanged. Added chrome key `owner.requested` (`Ø·ÙÙØ¨`) â the catalog missed the âRequested {time}â line.
...
**How to verify:** `/owner` after login â Ø§ÙÙÙÙ / Ø®Ø±ÙØ¬ / Ø§Ø­Ø¬Ø² Ø³Ø§Ø¹Ø© / ÙØ§ÙÙ; pitch and requester names as stored; Approve/Collect/Book still submit.

### In plain language

The ownerâs list on the wall is Arabic now. The WhatsApp note he sends is still English until the last step.

---

## Chapter 112 â 2026-09-12 â SPEC-13 step 5: WhatsApp Arabic body

**When:** 2026-09-12

**What:** `slotAvailableMessage` uses the pinned Arabic sentence. Stadium/pitch stay as stored; times stay Latin `18:00â19:00`. Jest updated. No other BR-71 templates. SPEC-13 is done in code.
...
**How to verify:** `npm test`. Notify `wa.me` text: `Ahmad Stadium: Pitch 1 18:00â19:00 Ø£ØµØ¨Ø­Øª ÙØªØ§Ø­Ø© ÙØ¬Ø¯Ø¯Ø§Ù Ø¥Ø°Ø§ ÙØ§ Ø²ÙØª ØªØ±ÙØ¯ÙØ§.`
...
**How to verify:** `npm test` (154). `/?tenant=ahmad` â HTML has `<bdi dir="ltr">16:00â17:00</bdi>` (start before end), 2-col grid, `ÙØ­Ø¬ÙØ²` on muted taken cells, **no** name/phone form until a chip is selected; Request still lands `ok=requested`. `/owner` Today/Book/waitlist ranges and phones no longer reverse. `/login?tenant=ahmad` slug is isolated.
...
## Chapter 114 â 2026-09-12 â Public day chips (Link, not a date form)

**When:** 2026-09-12

**What:** Public `/` no longer uses DateField + **Ø¹Ø±Ø¶ Ø§ÙØ³Ø§Ø¹Ø§Øª**. Same-week dates are a horizontal row of seven chips: Ø§ÙÙÙÙ, ØºØ¯Ø§Ù, the next four weekdays with a Western day number, then a calendar icon. The six day chips are Next.js `<Link href={{ pathname: "/", query: { tenant, date } }}>` (App Router client transition â not `<form method="get">`, not a raw `<a>`). The existing `<Suspense key={dateValue}>` around `PublicHours` is what should skeleton **only** the slot list. Window is always venue-todayâ¦today+5. A date outside that window rings the calendar chip (Popover + same `en-GB` Calendar); `router.push` with `tenant` + `date` only. `?date=` / `parseCivilDate` / `todayInTimeZone` unchanged.

**Why:** Same-day/this-week is the common path; a calendar-first field was two actions. No SPEC â UX, query contract unchanged.
...
**How to verify:** `npm test`. `/?tenant=ahmad` â six `/ ?tenant=ahmad&date=yyyy-mm-dd` Links, no DateField, no **Ø¹Ø±Ø¶ Ø§ÙØ³Ø§Ø¹Ø§Øª**; Today ringed; times still `16:00â17:00`. Tap ØºØ¯Ø§Ù â `?date=` tomorrow, that chip rings; stadium name + chips stay, **only** the hours area shows the skeleton. `/?tenant=ahmad&date=2026-10-01` â calendar chip `aria-pressed="true"`. `/owner` DateField still there.
...
## Correction â 2026-09-12 â Day-chip selected ring was clipped

**When:** 2026-09-12

**What:** `overflow-x-auto` on the chip row clipped the Volt ring (CSS treats the other axis as `auto` too). Padding on the row is now `p-2.5` so the ring+offset has room, including on Ø§ÙÙÙÙ at the RTL start edge.
...
**How to verify:** Select Ø§ÙÙÙÙ / ØºØ¯Ø§Ù â the full green ring is visible on all four sides, not cut by the row.

---

## Correction â 2026-09-12 â Day chips: no horizontal scrollbar
...
**How to verify:** `/?tenant=ahmad` â all seven chips visible, no scrollbar under the row; selected Ø§ÙÙÙÙ still has a full green outline.

---

## Correction â 2026-09-12 â Page scrollbar no longer shifts the layout
...
## Chapter 115 â 2026-09-12 â Public hours skeleton matches the slot grid

**When:** 2026-09-12

**What:** Replaced the two tall gray blocks. Hours Suspense now falls back to `PublicHoursSkeleton`: pitch heading bars at **inline-start** + a 2-col grid of time/price chip shapes (same `min-h-16` / `rounded-xl` as live slots). Pieces are exported separately (`StartLine`, `SlotChipSkeleton`, `SlotGridSkeleton`, `PitchHoursSkeleton`, `DayChipsSkeleton`) so later public UI can compose them. No `pl`/`pr`/`ml`/`mr` â `self-start` / `items-start` follow `dir`.
...
**How to verify:** Tap ØºØ¯Ø§Ù â stadium name and chips stay; hours area shows two pitch headings (start-aligned) and a 2-col chip grid, then real slots. Flip `dir` on `<html>` â heading bars and chip text stubs stay at inline-start.

---

## Chapter 116 â 2026-09-12 â Public EN/Ø¹ lang+dir toggle

**When:** 2026-09-12

**What:** Public header has a small outline button. Arabic â shows **EN**; English â shows **Ø¹**. Click sets cookie `stadium_locale`, flips `<html lang>` / `dir` (`ar`+`rtl` / `en`+`ltr`), and `router.refresh()` so public chrome (Today/Hours/Taken/form) follows. Stadium and pitch names stay as stored. No next-intl, no `/en` routes. Owner/login still Arabic copy (fallback).
...
**How to verify:** `/?tenant=ahmad` â EN at inline-end of the header. Tap â page goes LTR, Today/Hours/Request in English, chips at the left. Tap Ø¹ â back to RTL Arabic. `npm test` (157).

---

## Chapter 117 â 2026-09-12 â Colocate route UI; public form field errors

**When:** 2026-09-12

**What:** Public UI lives under `src/app/(public)/` (URL still `/`). Owner skeletons moved next to `/owner`. Shared primitives stay in `src/components/ui` (`LtrIsolate`, `FlashToast`). Public name/phone no longer use HTML `required` (browser bubble). `noValidate` + inline `role="alert"` under the field; same 8â15 digit phone rule as Zod. Server Action still parses with Zod.
...
**How to verify:** `npm test` (158). Open a slot, tap Ø§Ø·ÙØ¨ empty â red line under name and phone, no OS tooltip. Valid name + `03 123 456` still `ok=requested`.

---

## Chapter 118 â 2026-09-12 â Hide ended hours; three empty states

**When:** 2026-09-12

**What:** Public hours and Owner Book drop slots whose end is not after injected `now` (same cutoff as `booking.slot_ended`). `generateSlotsForDay` is unchanged. Empty list is `closed` (no windows), `past` (civil date before today), or `hoursEnded` (today, all games over). Public chip row stays todayâtoday+4; a past `?date=` selects nothing (not Today, not the calendar). Calendar disables days before today. Owner Today lists are unchanged. A typed past Owner `bookOn` shows `past` â Book creates, it does not review history.
...
**How to verify:** `npm test` (170). Public `/?tenant=ahmad&date=` yesterday â Â«ÙØ°Ø§ Ø§ÙØªØ§Ø±ÙØ® ÙØ¶ÙÂ», no chip ringed. Open calendar â days before today disabled. After last slot tonight â Â«ÙÙ ØªØ¨Ù Ø³Ø§Ø¹Ø§Øª Ø§ÙÙÙÙÂ». Closed weekday still Â«ÙØºÙÙ ÙØ°Ø§ Ø§ÙÙÙÙ.Â». Owner Book past `bookOn` â same past empty state; Owner Today still lists today's pending/confirmed.

---

## Chapter 119 â 2026-09-12 â Owner tabs: real routes + shared layout
...
**How to verify:** `npm test`. Login â `/owner/today`. Header stays while tapping Ø§Ø­Ø¬Ø² / ÙØ§Ø¦ÙØ© Ø§ÙØ§ÙØªØ¸Ø§Ø± / Ø§ÙÙØ§Ù. Book GET stays on `/owner/book`. Approve toast on Today; create booking toast on Book; rate/expense on Money.

---

## Correction â 2026-09-12 â Today lists live under today/lists.tsx
...
**How to verify:** Refresh `/owner/today` â pending/confirmed render; cancel form has no `keepOwnerQuery` error.

---

## Chapter 120 â 2026-09-12 â Owner tab bar icons + dock

**When:** 2026-09-12

**What:** Owner bottom nav is a floating rounded dock: Lucide icons (list / calendar-plus / bell / wallet) over the existing Arabic labels, Volt tint + slight scale on the active tab, CSS `transition` only. Still `<Link>` + `usePathname` â not a client tab switcher.
...
**How to verify:** Open `/owner/today` â dock at the bottom. Tap Ø§Ø­Ø¬Ø² â icon tints Volt, header stays. Waitlist label may truncate; icon still readable.

---

## Chapter 121 â 2026-09-12 â Colocate owner tab files; split actions

**When:** 2026-09-12

**What:** Each owner tab keeps its own page, UI, skeleton, and (when it has a form POST) `actions.ts`. Root `/owner` keeps only chrome that two or more tabs share: `layout.tsx`, `tab-bar.tsx`, `shared.tsx` (membership, tenant slug, `queryString`, `formatLocalRange`), and `form-query.ts` (`field` / `ownerQuery` / `redirectOwner` used by today+book+money). One-tab helpers are folded into that tabâs TSX (`keepTenantQuery` on Today, `keepBookQuery` on Book, period keep/format/labels on Money). `civilFromYyyyMmDd` / `parseOwnerBookOn` live in `book/date.ts`. Waitlist has no Server Action.

**Why:** A root `actions.ts` and `skeletons.tsx` mixed four products. Next tab (Shop) would dump more into the same files. Rule: used by exactly one tab â lives in that folder.
...
**How to verify:** `npm test`. Open `/owner/today` â approve still toasts on Today. Book a slot â stay on `/owner/book` with `bookOn`. Rate/expense still land on `/owner/money`. No import of `@/app/owner/actions`.

---

## Chapter 122 â 2026-09-12 â Owner chrome: waitlist cards, quieter header, Money heading

**When:** 2026-09-12

**What:** Waitlist people use the same card anatomy as Today (pitch + time + name/phone in `CardHeader`, Ø¥Ø¨ÙØ§Øº full-width in `CardContent`). Shared owner header no longer uses a `text-2xl` stadium `h1`; stadium + role are muted, logout is ghost icon-only. Money has a page `h2` Ø§ÙÙØ§Ù; period/rate/expenses are `h3` subsections like Today. Book/Waitlist page titles match Todayâs `text-xl`.
...
**How to verify:** Waitlist card â name/phone stacked under the time, Ø¥Ø¨ÙØ§Øº full-width like Ø¥ÙØºØ§Ø¡. Header: stadium small, tab title (Ø§ÙÙÙÙ / Ø§ÙÙØ§Ù) is the big type. Money opens with Ø§ÙÙØ§Ù then ÙØ°Ù Ø§ÙÙØªØ±Ø©.

---

## Chapter 123 â 2026-09-12 â Shared SlotPicker for public hours and owner Book

**When:** 2026-09-12

**What:** Public and owner Book share one client `SlotPicker`: 2-col time+price grid, one expanded name/phone form. Props are `action`, `hiddenFields`, `submitLabel`, optional `locale`. Local pitch/slot view types â no `booking/`, access, or venue imports. Thin wrappers: `PublicSlotPicker` (`tenant`+`date`, Ø§Ø·ÙØ¨) and `OwnerSlotPicker` (`tenant`+`bookOn`, Ø§Ø­Ø¬Ø²). Name/phone client checks moved to `src/lib/request-fields.ts`. Owner Book no longer stacks a form on every free slot. Auth stays on `/owner/book`.
...
**How to verify:** `npm test`. Public â tap a free hour, one form, Taken muted, Request still `ok=requested`. Owner Book â same grid, one form, stays on `/owner/book` with `bookOn`. Staff without create still EmptyState. EN/Ø¹ still only public labels.

---

## Chapter 124 â 2026-09-12 â Owner Book uses public day chips

**When:** 2026-09-12

**What:** Owner Book date is the same five Link chips + calendar overflow as public hours. Shared `DayChips` / `DateCalendarChip` take `pathname` + `dateQueryKey` (`date` on `/`, `bookOn` on `/owner/book`). The GET date form and Ø¹Ø±Ø¶ Ø§ÙØ³Ø§Ø¹Ø§Øª button are gone. Chips sit outside `<Suspense>` so they stay mounted while slots load. Public `PublicDayChips` is a thin wrapper.
...
**How to verify:** `npm test`. `/owner/book` â today/tomorrow chips, calendar for later days, slots update without a submit. Create booking still returns to the same `bookOn`. Public `?date=` chips unchanged.

---

## Chapter 125 â 2026-09-12 â SlotPicker v2: kickoff hierarchy, Volt duration, select motion

**When:** 2026-09-12

**What:** Slot tiles: start time is the hero; end is a smaller Slate line (`â 17:00`); price + duration share the third line. Idle duration is the only Volt accent. Idle fill is `bg-card` plus a 1px `#dee2e6` hairline (`border-border` on these tiles only â `#ffffff` vs page `#f8f9fa` is ~1.02:1 and would vanish in sun). Selected is a 200ms ease-out fill-invert (no ring, no hover, no shadow). Taken is the same stack, muted, instant. No globals / `--border` change.

**Why:** Equal-weight `16:00â17:00` plus a snap invert read as a flat box. Kickoff is the number that matters; duration is real info (1h / 1.5h).
...
**How to verify:** `npm test`. Public/owner Book â idle tile: large 16:00, small â 17:00, `$30.00 Â· 1h` with Volt on `1h`. Tap â Volt fill over ~200ms. Taken: no Volt, ÙØ­Ø¬ÙØ². Day chips still have their own border.

---

## Chapter 126 â 2026-09-12 â Idle tile Slate hairline; light theme-color

**When:** 2026-09-12

**What:** Idle slot tiles use a 1px Slate hairline (`border-muted-foreground/40`, `#495057` at 40%) â not `#dee2e6`, not `shadow-sm`. Selected stays Volt invert (`border-primary`); taken stays the faint `border-border`. Root viewport is `themeColor: #f8f9fa` + `colorScheme: light` (local `generate-viewport.md`; Next default `themeColor` is null). `html` paints `--background` and `color-scheme: light` so OS-dark chrome / canvas does not show a black strip above the header.

**Why:** Chapter 125âs `#dee2e6` hairline was still invisible on-screen (sunlight risk confirmed). The black bar was not a tile style â missing theme-color on a light page.
...
## Chapter 127 â 2026-09-12 â Slot form is a dialog; ticket split on every tile
...
**Why:** In-grid `col-span-2` was the smallest âone formâ fix; it shoved stubs once tiles became tickets. Modal keeps the board still.
...
**How to verify:** `npm test`. Public / owner Book â tap a free stub: grid stays put, tile inverts, dialog has the ticket face + name/phone. Overlay or Esc closes. Taken tiles use the same split. Request / Ø§Ø­Ø¬Ø² still `ok=`.

---

## Chapter 128 â 2026-09-12 â Scrollbar gutter both-edges; dialog does not open a white strip

**When:** 2026-09-12

**What:** `html { scrollbar-gutter: stable both-edges }` so `mx-auto` stays optically centered and the classic Windows bar never covers content. Dialog overlay is `left-0 w-screen` (covers the gutter). RemoveScrollâs extra `margin/padding-right` on `body[data-scroll-locked]` is zeroed â that gap was the white column beside the dim.

**Why:** `stable` alone reserved only the barâs side, so the column sat off-center. Opening the dialog hid the bar and RemoveScroll added a second gap on top of the gutter.
...
**How to verify:** Long Book page â column centered with or without a bar. Open a slot â dim covers the full viewport, no white strip, page behind does not jump.

---

## Chapter 129 â 2026-09-12 â Thin Volt scrollbar; drop both-edges gutter
...
**Why:** Both-edges reserved matching empty space opposite the bar â the new white columns. A thin Volt bar matches the rest of the UI and needs only one thin lane.

**Files:** `src/app/globals.css`.

**Relation:** Dialog dim still `w-screen`. No token change.

**How to verify:** Book page â thin Volt bar, no empty strip on the other side. Open a slot â dim edge to edge.

---

## Chapter 130 â 2026-09-12 â Drop scrollbar-gutter; overlay spans 100vw
...
**How to verify:** Open a slot on Book â dim is edge to edge, no white column. Scrollbar is a thin Volt pill, not a reserved white lane.

---

## Chapter 131 â 2026-09-12 â Owner header EN/Ø¹ toggle

**When:** 2026-09-12

**What:** Owner header has the same EN/Ø¹ control as public (`LangToggle`, cookie `stadium_locale`, `html` lang/dir). Shared component; `setUiLocale` lives at `src/app/locale-actions.ts`. Owner chrome, tabs, and tab pages pass `locale` into `ui()`. English strings cover owner / empty / role / category keys. Stadium and pitch names stay as stored.
...
**How to verify:** `npm test`. `/owner/today` â EN in the header; tabs and Today copy go English + LTR. Ø¹ returns Arabic + RTL. Public `/` still shares the cookie.

---

## Chapter 132 â 2026-09-13 â Home tab (replace Today inbox)

**When:** 2026-09-13

**What:** `/owner/today` is the Home tab (Ø±Ø¦ÙØ³ÙØ© / Ø§ÙØ±Ø¦ÙØ³ÙØ©). Two sections, not a merged timeline. **Ø·ÙØ¨Ø§Øª:** all PENDING any date, soonest slot then BR-17 `requestedAt` inside a slot; competing requesters grouped on one card with approve/reject always visible. **Ø§ÙÙØ§Ø¯Ù:** compact confirmed rows (time, pitch, name, ÙØ§Ø¯Ù/ÙØ³ØªØ­Ù/ÙØ¯ÙÙØ¹); tap-in Collect / Ø¯ÙØ¹ Ø¨Ø¹ÙÙØªÙÙ / Cancel. Today by default; **Ø¹Ø±Ø¶ Ø§ÙØ£ÙØ§Ù Ø§ÙÙØ§Ø¯ÙØ©** appends the next 7 Beirut civil days. **ÙØªØ£Ø®Ø±** prepends unpaid APPROVED before today (BR-49). URL stays `/owner/today`. Server Actions unchanged.

**Why:** Owner IA â ops inbox vs compact upcoming. BR-49 forbids dropping past unpaid with no collect path.
...
**How to verify:** `npm test`. `/owner/today` â tab House + Ø±Ø¦ÙØ³ÙØ©; pending any date with inline date; competing slot shows all requesters + buttons; today compact tags; tap row for collect/cancel; overdue unpaid above today; coming-days toggle. Staff without approve/collect: lists without those forms.

---

## Chapter 133 â 2026-09-13 â Home collect: due vs remaining figures

**When:** 2026-09-13

**What:** Tap-in collect detail shows due and remaining as two labeled tiles instead of one `Due $30 Â· remaining $30` sentence. Remaining uses Volt when still owed; both quiet when remaining is `$0.00`.
...
**How to verify:** `/owner/today` â expand a confirmed row. Due is Slate tile; remaining is Volt if unpaid. Paid row: remaining muted.

---

## Chapter 134 â 2026-09-13 â Home cards: time hero, icons, real status pills

**When:** 2026-09-13

**What:** Home request and upcoming cards: time is the scan target (`text-lg semibold` + Clock). Pitch is MapPin + muted; phone is Phone + LTR (requests + expanded detail). Status tags are `Badge outline` in both locales â ÙØ§Ø¯Ù/ÙØ¯ÙÙØ¹ were `ghost` (no pill). Paid includes a Volt `CircleCheck`. Dropped the extra âpendingâ pill on competing requesters (section is already Ø·ÙØ¨Ø§Øª). Weekday labels are short in both locales.
...
## Chapter 135 â 2026-09-13 â Toast above the tab bar
...
**How to verify:** Approve or collect on `/owner/today` â toast appears under the status bar, not over Ø±Ø¦ÙØ³ÙØ© / Ø§Ø­Ø¬Ø².

---

## Chapter 136 â 2026-09-13 â No Cancel on past unpaid (BR-49)
...
**How to verify:** `npm test`. Home â expand a due/overdue row after kickoff: Collect, no Cancel. Future unpaid: Cancel still there. Paid past: Cancel still there. Stale POST â toast `booking.cancel_past_unpaid`.

---

## Chapter 137 â 2026-09-13 â Toast follows locale; dismiss X
...
**How to verify:** `npm test`. Switch to English, approve/collect â toast is English. X dismisses it. Arabic still Arabic.

---

## Chapter 138 â 2026-09-13 â Reports tab + More / Settings

**When:** 2026-09-13

**What:** Bottom bar is five tabs: Home / Book / Waitlist / Reports / More. `/owner/money` stays (heading + tab = ØªÙØ§Ø±ÙØ± / Reports); period + expenses stay; exchange-rate **set** form moved to `/owner/more/settings`. More is a hub (Settings only for now). Waitlist tab label shortens to Ø§ÙØªØ¸Ø§Ø±; page heading stays ÙØ§Ø¦ÙØ© Ø§ÙØ§ÙØªØ¸Ø§Ø±.

**Why:** Rate is stadium config, not a report. More is the reserved home for Settings extras, Tournaments, Shop, Academy â the bar does not grow again.
...
**How to verify:** `npm test`. Owner bar shows 5 labels including ØªÙØ§Ø±ÙØ± and Ø§ÙÙØ²ÙØ¯. Reports has period + expenses, no rate form. More â Settings: OWNER can set rate; toast lands on Settings. Waitlist tab says Ø§ÙØªØ¸Ø§Ø±; heading is still ÙØ§Ø¦ÙØ© Ø§ÙØ§ÙØªØ¸Ø§Ø±.

---

## Chapter 139 â 2026-09-13 â Reports Pass 1: hero, disclosure, expense rows

**When:** 2026-09-13

**What:** Reports summary: difference is `text-2xl`; In tile Volt, Out tile due-muted; CSS In/Out bars from the two totals (no library). Period GET form behind âChange periodâ; record-expense form behind âAdd expenseâ. Expense history is compact cards (lucide category icon, description, LTR date, mono amount). Default period remains this Beirut calendar month.

**Why:** Home-style priority disclosure. The form and raw em-dash list buried the number. SPEC-08 said no JS chart â this is two CSS bars, not a series.
...
**Relation:** `summarizeLedgerPeriod` / `recordExpense` / `listRecentExpenses` unchanged. No new query. Out color may still read as âneeds actionâ (Home due) â revisit after live use, not a new token now.

**How to verify:** `npm test`. `/owner/money` â net is the largest figure; In/Out tiles; bars; âØªØºÙÙØ± Ø§ÙÙØªØ±Ø©â / âØ¥Ø¶Ø§ÙØ© ÙØµØ±ÙÙâ collapsed. Expense rows are cards, not an em-dash sentence.

---

## Chapter 140 â 2026-09-13 â Pitch MVP: daily window + flatten/pending confirm

**When:** 2026-09-13

**What:** Owner Settings lists pitches and (OWNER only) create/edit: name, one open/close copied to all 7 days, slot duration, default USD. Writes go through `parseScheduleConfig` with `gapMinutes: 0` and `priceRules: []`. Hours-cover (not generated-slot match) refuses live APPROVED in a removed window; PENDING in a removed window and flatten-of-mixed-days/`priceRules` use the same checkbox second-submit (`confirmPending` / `confirmFlatten`). Duration-only changes do not trip hours-cover. `approveBooking` re-checks `resolveOfferedSlot` against current hours before flipping PENDING â APPROVED.

**Why:** Seed was the only pitch writer. Flatten must not silently wipe Ahmad A1 Fri/Sat hours and weekend `priceRules`. DR-001: Venue does not import Booking â `listLiveWindowsOnPitch` feeds `{ start, end, status }[]` into `updatePitch`.
...
**How to verify:** `npm test`. More â Settings: pitch list (A1 = Ø³Ø§Ø¹Ø§Øª ÙØ®ØªÙÙØ©). Edit A1 â flatten checkbox visible; save without it toasts `venue.confirm_flatten` and does not write. Check it, save â all 7 days match the form window, `priceRules` empty. Shrink hours over a live APPROVED â refuse. Over PENDING only â checkbox then save (request stays PENDING). Approve a request whose hour was closed â `booking.slot_not_offered`, still PENDING.

---

## Chapter 141 â 2026-09-13 â Day priceRules editor; flatten is hours-only

**When:** 2026-09-13

**What:** Pitch create/edit can add repeatable day-checkbox + USD `priceRules` (cap 10). Flatten retargeted to **hours mismatch only** â weekend prices no longer trip the confirm, and a duration/default-price save with unchanged hours keeps existing rules. `gapMinutes` stays hardcoded 0. Time-of-day rule clocks are not in the UI; stored `start`/`end` round-trip in the JSON draft if already present.
...
**How to verify:** `npm test` (includes duration+price-only save keeps weekend rules and `hoursSaveBlocker` is null). Edit a uniform-hours pitch with Fri/Sat $40 â no flatten checkbox; change duration or default USD; save; public/book still shows $40 on those days. Mixed-hours A1 still shows flatten for hours.

---

## Chapter 142 â 2026-09-13 â Price-rule checkbox ids must not use a module counter
...
**How to verify:** Open `/owner/more/settings/pitches/[pitchId]` for A1 â no hydration warning in the console. Day checkboxes still toggle.

---

## Chapter 143 â 2026-09-13 â Hours groups; flatten retired

**When:** 2026-09-13

**What:** Pitch create/edit replaces the single Opens/Closes pair with repeatable hours groups (weekday checkboxes + one open/close per row). Collapse walks `WEEKDAYS`, skips `[]`, and groups identical window arrays â A1 is two rows (MonâThu 16:00â22:00, FriâSat 16:00â23:00) plus Sun closed. A day in two rows is disabled in the UI and rejected as `venue.hours_day_overlap`. Flatten (`confirmFlatten` / `needFlatten` / `venue.confirm_flatten`) is gone. Pending-in-removed-hours confirm stays. `gapMinutes` stays 0. Known limit: schema still allows several windows per day; the editor and collapse use `window[0]`.
...
**How to verify:** `npm test`. Edit Ahmad A1 â two hours rows + Sun closed; Fri/Sat $40 still in Day Prices; no flatten checkbox. Duration-only save keeps hours groups and weekend prices. Duplicate day â `venue.hours_day_overlap`. Shrink hours over live APPROVED â refuse; PENDING-only â confirm then save.

---

## Chapter 144 â 2026-09-13 â SPEC-14 no-show (BR-22)

**When:** 2026-09-13

**What:** Fast-forwarded `main` to include Settings / hours groups, branched `feature/spec-14-no-show`. Owner (or `"bookings.no_show"`) records APPROVED â `NO_SHOW` after the hour has ended. Unpaid no-show stays on Home overdue/today so Collect still works (`assertCanCollect` allows `NO_SHOW`). Paid no-show leaves Home. Cancel, waitlist, exclusion, and ledger writes are unchanged. No confirm dialog.

**Why:** BR-22 wants âdidnât happenâ distinct from cancel. Cancel already refuses past unpaid (BR-49 / ch.136); no-show is that close-out without dropping the debt. DR-002 Â§2.12: status, not a boolean. SPEC-14.
...
**How to verify:** `npm test`. Home â after a finished unpaid APPROVED, expand: Collect + ÙÙ ÙØ­Ø¶Ø±, no Cancel. Tap no-show; still Collect. Collect remaining â row leaves. Future row: no No-show. `staff@ahmad`: no button. Stale POST before end â `booking.no_show_not_ended`.

---

## Chapter 145 â 2026-09-14 â Confirm-notify WhatsApp on Home

**When:** 2026-09-14

**What:** After Approve, Homeâs confirmed row can Ø¥Ø¨ÙØ§Øº via `wa.me` with a âbooking confirmedâ Arabic body. Link is a static `<a>` (same as waitlist), not a Server Action. Expand groups Ø¥Ø¨ÙØ§Øº above ØªØ­ØµÙÙ. Successful Approve redirects `ok=approved&highlight=<id>`; FlashToast still strips only `ok`; the row keeps a primary ring (coming-days opens if that id is in `later`). No auto-expand. Owner Book and reject-notify out.

**Why:** BR-71 âbooking confirmedâ. Players have no login. Waitlist Notify is the losersâ hour-freed message; the winnerâs confirm belongs on Homeâs APPROVED expand. `approveBooking` is unchanged â notify is after the fact.
...
**How to verify:** `npm test`. Login `owner@ahmad` â pending Request â Approve â toast + ring on that Ø§ÙÙØ§Ø¯Ù row (`highlight=` stays in the URL). Expand: Ø¥Ø¨ÙØ§Øº then Collect/Cancel. Link opens WhatsApp; text is `{stadium}: {pitch} {start}â{end} ØªÙ ØªØ£ÙÙØ¯ Ø­Ø¬Ø²Ù.` Unpaid NO_SHOW: no Ø¥Ø¨ÙØ§Øº. Coming-days approve: that section is open, row may still be off-screen (no scrollIntoView yet).

---

## Chapter 146 â 2026-09-14 â Home confirmed details in a bottom sheet

**When:** 2026-09-14

**What:** Confirmed Home rows stay compact. Tap opens a shared bottom sheet (slide up, overlay, scroll inside) with Ø¥Ø¨ÙØ§Øº + Collect/Cancel/No-show. Rows show a trailing chevron (`rtl:rotate-180`), hover/active wash, and `cursor-pointer` so they read as tappable. `BottomSheet` is a general Dialog-based component â same overlay/RemoveScroll rules as the centered dialog; no extra library.
...
**How to verify:** `npm test`. Home â compact confirmed rows with chevron. Tap: sheet slides up over the tab bar; overlay tap / Esc / close dismisses. Collect and Ø¥Ø¨ÙØ§Øº still work in the sheet. RTL: chevron points toward the start edge.

---

## Chapter 147 â 2026-09-14 â Cancel booking is not âclose the sheetâ

**When:** 2026-09-14

**What:** Home sheet no longer has a full-width outline **Ø¥ÙØºØ§Ø¡** that reads as dismiss. The sheet still closes with X / overlay / Esc (`Ø¥ØºÙØ§Ù`). Booking cancel is a separate ghost **Ø¥ÙØºØ§Ø¡ Ø§ÙØ­Ø¬Ø²**, then a red **ØªØ£ÙÙØ¯ Ø¥ÙØºØ§Ø¡ Ø§ÙØ­Ø¬Ø²** (hint: hour is freed) with **ØªØ±Ø§Ø¬Ø¹**. `cancelBooking` is unchanged.

**Why:** In a bottom sheet, âCancelâ is the usual name for go-back. One tap was flipping APPROVED â CANCELLED forever. The owner (and the person who built it) hit it by mistake.
...
**Relation:** No domain/status-rule changes. No-show stays one tap (`ÙÙ ÙØ­Ø¶Ø±` does not mean close).

**How to verify:** `npm test`. Open a confirmed row â X / dim / Esc close the sheet, booking stays APPROVED. Ø¥ÙØºØ§Ø¡ Ø§ÙØ­Ø¬Ø² does not submit; ØªØ£ÙÙØ¯ Ø¥ÙØºØ§Ø¡ Ø§ÙØ­Ø¬Ø² does. ØªØ±Ø§Ø¬Ø¹ returns to the quiet button.

---

## Chapter 148 â 2026-09-14 â Cancel confirm replaces the sheet, does not grow it

**When:** 2026-09-14

**What:** Tap Ø¥ÙØºØ§Ø¡ Ø§ÙØ­Ø¬Ø² swaps the sheet to a short confirm view (title, which booking, hint, ØªØ£ÙÙØ¯ / ØªØ±Ø§Ø¬Ø¹). Collect / Ø¥Ø¨ÙØ§Øº go away until ØªØ±Ø§Ø¬Ø¹. Confirm buttons stay above the screen edge; no extra scroll. Close / open another row resets the step.

**Why:** Appending the red confirm under Collect made a already-tall sheet taller; ØªØ£ÙÙØ¯ sat under the fold.
...
**How to verify:** Open a confirmed row with Collect visible â Ø¥ÙØºØ§Ø¡ Ø§ÙØ­Ø¬Ø² â sheet shrinks; ØªØ£ÙÙØ¯ and ØªØ±Ø§Ø¬Ø¹ are on-screen without scrolling. ØªØ±Ø§Ø¬Ø¹ restores Collect. X still only closes.

---

## Chapter 149 â 2026-09-14 â Smooth sheet height + close
...
**How to verify:** Open a confirmed row â sheet slides up. Ø¥ÙØºØ§Ø¡ Ø§ÙØ­Ø¬Ø² â sheet shrinks, buttons fade in. ØªØ±Ø§Ø¬Ø¹ â sheet grows back. X / dim â sheet slides down. Reduced-motion: instant swap, no height tween.

---

## Chapter 150 â 2026-09-14 â Cancel step: inert, not aria-hidden on focused button

**When:** 2026-09-14

**What:** Hidden sheet step uses `inert` only (no `aria-hidden`). Tap Ø¥ÙØºØ§Ø¡ Ø§ÙØ­Ø¬Ø² blurs first, then focus moves to ØªØ£ÙÙØ¯ Ø¥ÙØºØ§Ø¡ Ø§ÙØ­Ø¬Ø²; ØªØ±Ø§Ø¬Ø¹ returns focus to Ø¥ÙØºØ§Ø¡ Ø§ÙØ­Ø¬Ø².

**Why:** Chrome blocked `aria-hidden` on the details panel because Ø¥ÙØºØ§Ø¡ Ø§ÙØ­Ø¬Ø² still had focus. Spec: donât hide a focused descendant; `inert` also prevents focus.

**Files:** `src/app/owner/today/upcoming-panel.tsx`

**Relation:** Same two-step cancel. No domain change.

**How to verify:** Open a confirmed row â Ø¥ÙØºØ§Ø¡ Ø§ÙØ­Ø¬Ø². Console should not show the aria-hidden focus warning. Focus ring lands on ØªØ£ÙÙØ¯; ØªØ±Ø§Ø¬Ø¹ lands on Ø¥ÙØºØ§Ø¡ Ø§ÙØ­Ø¬Ø².

---

## Chapter 151 â 2026-09-14 â Home sheet hierarchy and mixed-pay disclosure

**When:** 2026-09-14

**What:** Confirmed booking sheet: time is SlotFace-scale; pitch + name one muted line; phone is a small header icon+number. Due/remaining collapse to one remaining hero when they match; two tiles only after a partial. Ø¥Ø¨ÙØ§Øº is ghost above a divider; Collect is the primary. USD/LBP hide behind Ø¯ÙØ¹ Ø¨Ø¹ÙÙØªÙÙ (one-tap Collect unmounts while open). Mixed USD is remaining, labeled Ø§ÙÙØªØ¨ÙÙ Ø¨Ø§ÙØ¯ÙÙØ§Ø±, not a $30 placeholder.
...
**How to verify:** `npm test`. Unpaid equal price/remaining: one hero, one Collect, no currency fields until Ø¯ÙØ¹ Ø¨Ø¹ÙÙØªÙÙ. Open mixed: one-tap gone; USD prefilled remaining. Partial: two tiles. Paid: no Collect. RTL: no `pl`/`pr`.

---

## Chapter 152 â 2026-09-14 â Sheet start-edge, phone+Ø¥Ø¨ÙØ§Øº, mixed outline

**When:** 2026-09-14

**What:** Booking sheet copy and amounts use `text-start`; remaining hero/tiles drop extra `px` so `$` sits on the same start edge as ØªØ­ØµÙÙ. Ø¥Ø¨ÙØ§Øº is a compact outline on the phone row (not its own section). Ø¯ÙØ¹ Ø¨Ø¹ÙÙØªÙÙ is a full-width outline button; mixed disclosure (unmount one-tap, remaining USD) is unchanged.

**Why:** `$25.00` read as centered; Ø¥Ø¨ÙØ§Øº did not belong to the number; ghost `self-start` Ø¯ÙØ¹ Ø¨Ø¹ÙÙØªÙÙ looked like a label.

**Files:** `src/app/owner/today/upcoming-panel.tsx`; `docs/owner-ia.md`.

**Relation:** Same `wa.me` `<a>` and `submitCollectPayment`. No domain change.

**How to verify:** Open a confirmed APPROVED row. Phone and Ø¥Ø¨ÙØ§Øº share a row. Time, remaining, Collect, Ø¯ÙØ¹ Ø¨Ø¹ÙÙØªÙÙ, Cancel share the RTL start edge. Ø¯ÙØ¹ Ø¨Ø¹ÙÙØªÙÙ has a border; tap still reveals USD/LBP and hides one-tap Collect.
...
## Integration tests ï¿½ dedicated prisma dev instance + Phase 1 cases
...
**Gotcha:** Local `prisma dev` **ignores the database name** in the URL and always serves `template1` for that named instance. `CREATE DATABASE stadiums_test` on the demo proxy does **not** isolate ï¿½ early truncates wiped demo data (reseeded). Isolation = `prisma dev --name stadiums-test` on TCP **:51218**, never demo **:51214**. Local proxy is **single-connection**; truncate must use the app Prisma pool (no second `pg.Pool`); concurrent dual `` is not viable for the TOCTOU race ï¿½ mapping test uses optional `deps.listApprovedRanges`.
...
**How to verify:** `/owner/money` with negative Difference shows `-$â¦`; Add expense opens sheet; list rows show tinted icons.

## Venue: atomic `updatePitch`

**When:** 2026-09-15

**What:** `updatePitch` wraps find â hours-cover classify â update in one `db.$transaction`. `findPitch` / `updatePitchRow` take `tx: TenantTx` (pass interactive `tx` or top-level `db`). `findPitchById` is a same-select alias for Booking call sites. Left `listPitches` / `insertPitch` / Access memberships on `db` (low near-term risk).

**Why:** Only realistic near-term mid-transaction Venue trigger from the engineering-audit footgun analysis â Settings hours save should be atomic.

**Files:** `src/modules/venue/infrastructure/pitches.ts`, `application/update-pitch.ts`, `application/get-pitch-editor.ts`.

**How to verify:** Edit pitch hours in Settings and save; approve/public booking still use `findPitchById(tx, â¦)`.

## Priority 3 dedup â starting
...
**How to verify:** `npm test` â 224 passed.
...
**How to verify:** Open the guide and walk the tree under `src/` â listed paths should exist; commented âno â¦â notes should stay absent.
...
**Why:** Need evidence before claiming âevery error has an expected shapeâ / âwe can debug production from logs.â

**Files:** `docs/guides/error-handling-logging-audit.md` (new).

**How to verify:** Open that guide â every claim cites file/line; ordered P0âP3 list at the end.

## P0: logout action wrap + WhatsApp soft-fail logs

**When:** 2026-09-15

**What:** (1) `submitLogout` try/catch + `actionErrorKey` â `?error=` on login (same shape as `submitLogin`). (2) Home confirm + waitlist WhatsApp catch blocks `logger.info` with useCase + tenantId, still return null (no phone in log).

**Why:** Error-handling audit P0 â DR-004 action boundary + âlog side-effect failuresâ; RULE-9 soft-fail unchanged. `info` not `error`: bad phone is expected data, not a system bug.

**Files:** `src/app/login/actions.ts`, `list-due-bookings.ts`, `list-open-waitlist.ts`.

**How to verify:** `npm test` â 224 passed. Force logout DB failure â login `?error=`; bad phone on Home/waitlist â null href + line in `logs/`.
...
**Why:** Error-handling audit P1 â Prisma failures on reads must hit the logger; success events must be greppable by tenant/useCase on the droplet.

**Files:** 11 read wraps + 12 info-context updates (23 unique application files).

**How to verify:** `npm test` â 224 passed. Approve/collect â `logs/` line includes `useCase=` + `tenantId=`.

## Audit: MVP readiness (read-only)

**When:** 2026-09-15

**What:** Full launch-readiness audit â BRD Phase 1 traceability, money integrity, isolation (unproven by automation), droplet performance, i18n/RTL, docs drift, deploy, fresh YAGNI/DRY. No code fixes. Isolation test not run (fixture needs parameterization).

**Why:** Need an honest âhand to a paying owner?â answer with file evidence, not architecture vibes.

**Files:** `docs/guides/mvp-readiness-audit.md` (new).

**How to verify:** Open that guide â eight sections + ordered P0âP3 must-close list; verdict is pilot-with-supervision, not production-ready.

## Isolation integration test (RULE-7) + tenant guard fix

**When:** 2026-09-15

**What:** Parameterized `seedMinimalFixture` + `seedTwoTenants`. New `test/integration/isolation.integration.test.ts` â under Aâs context: findUnique null for Bâs pitch, listPitches excludes B, update throws + B unchanged, `requestPublicSlot` â `booking.pitch_not_found` with no B booking. Guard fix in `db.ts`: inject `tenantId` into findUnique select when omitted; **pre-check** update/delete via `findFirst({ ...where, tenantId })` (post-check was too late â write already committed).

**Why:** MVP readiness P0 â prove RULE-7 in practice. First run exposed that `findPitch`/`updatePitchRow` selects without `tenantId` skipped the post-check, so cross-tenant update succeeded.

**Files:** `test/integration/fixtures.ts`, `isolation.integration.test.ts`, `src/lib/db.ts`.

**How to verify:** `npm run test:integration` â 11 passed (4 suites).

## Docs consolidation â NOW.md entry point
...
**Why:** `ui-components.md` â a flow with inputs is a sheet. Structural move before the slot restyle, while the diff is only the wrapper.
...
**How to verify:** Public `/` on a tenant host â tap an open hour. Name and phone slide up from the bottom; close clears the selection.

## Typography roles: Manrope, stacks, Arabic scale

**When:** 2026-09-23

**What:** Manrope (400â800, latin) is `--font-manrope`. `font-sans` is Manrope then Plex Arabic. `font-heading` and `font-display` are Big Shoulders, then Kufi, then Plex Arabic. `:lang(ar)` sets looser line-height, steps text-xs/sm/base, and forces `letter-spacing: normal`. Root font-size is unchanged.
...
**How to verify:** `/owner/book` â body font Manrope; a slot timeâs computed family is `\"Big Shoulders\"` and `document.fonts.check('700 16px \"Big Shoulders\"')` is true. English `tracking-tight` is negative; Arabic is `normal`.

## Slot tile: inverse fill, until-word, muted duration

**When:** 2026-09-23

**What:** Slot presentation only. Idle tile is inverse in light and surface in dark. Selected tile is an accent fill with carbon ink. End time uses `public.until` (Ø­ØªÙ / until). Duration is a surface-2 micro pill with muted ink. Unavailable is a stripe and not a button.
...
**How to verify:** `/owner/book` light and dark, Arabic and English. Time is the largest volt figure on a dark tile; price is second; `1h` is a muted pill; the end line says Ø­ØªÙ or until.
...
**How to verify:** `/owner/book` â light selected cell is carbon with off-white ink; dark selected cell is volt with carbon ink. No ring.
